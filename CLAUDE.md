...
- **Safety Rules** in prose (Allowed / Forbidden). **Never write outside `_local/`** — only exceptions: source-mutating skills (`implement`, `verify-fix`, `qa-followup`), `qa-host`, and `ship` (scoped: the Phase 4.2 CI-remediation loop only). `commit`/`pr` are the only delivery-writing skills; destructive VC operations stay forbidden everywhere.
