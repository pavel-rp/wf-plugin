...
| `/wf:qa-gen` | Generate a QA plan for the task (`06_qa.md`). UI criteria become **browser** scenarios; backend criteria become **API** scenarios that exercise the endpoint over HTTP with a real token. A **migration** task also gets a **Migration parity** suite checking the migrated Angular part against its legacy C#/MVC counterpart for functional and visual parity. Every plan ends with a standing **Baseline health** suite. A backend-only task is never a stub PASS. |
