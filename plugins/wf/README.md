...
| `/wf:resolve` | Inspects and manages the wf resolution snapshot through the bundled typed resolver **MCP service** — the same typed tools (`resolve_config`/`resolve_registry`/`resolve_provider`/`resolve_profile`/`resolve_plugin_root`, `resolve_gate`, `inspect_pack`/`register_pack`, and the authoring validators `validate_manifest`/`validate_registry`/`validate_skill_interface`) that normal skills and isolated subagents call to obtain config, capability, provider, path, and diagnostic facts **without any plugin-root probe or installed-folder walk**. Dispatches `inspect` (state + diagnostics + resolved view), `refresh` (rebuild the resolved snapshot), and `invalidate` (mark it stale). The public query responses carry paths/metadata/diagnostics only — **never a fragment body**. When the resolver is broken (missing / malformed / schema-incompatible / fingerprint-unresolvable / cli-unavailable / registry-invalid), `resolve_gate` binds the failure to the existing surface policy — a **local read continues**, a **tracker write warns**, a **delivery write blocks before any mutation** — with categorized diagnostics + a `/wf:resolve refresh`/`invalidate` recovery, never a folder/environment probe. Use to check what wf resolved, force a rebuild after editing the registry or (un)installing a pack, or debug a capability that isn't resolving. The three **authoring validators** are the in-session twin of the CI shell guards (`validate-registry.sh`, `skill-slot-marker-lint.sh`): they check an authored capability manifest, the registry, and a skill's slot markers against the **live** contract artifacts — deriving the phase spine, contribution taxonomy, and merge policies from `capability-registry.ops.md` on every call rather than transcribing them — and return a typed verdict (`pass`/`fail`/`error`) with per-finding file/line diagnostics under the guards' own check ids (`CHECK-1`…`CHECK-9`, `D1`…`D5`). A broken input returns a typed `error`, never a crash and never a silent pass. The shell guards remain authoritative in CI; the tools **agree** with them rather than replacing them. |
