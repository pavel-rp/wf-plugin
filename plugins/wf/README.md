...
Every routed unit calls the body-free `resolve_routing` MCP query immediately before execution. The caller supplies typed `shapeEvidence` about work surface, atomicity, independence, ambiguity, risk, tool work, validation, isolation value, review need, return contract, and requested parallelism — never a caller-selected execution shape. The resolver returns `inline`, `isolated`, or `bounded-parallel`; callers must obey it exactly and stop before work on any diagnostic. Bounded-parallel work completes every independent unit while limiting concurrency to the minimum of unit count, the positive caller bound, and the core maximum of four. It also resolves host enforcement → invocation override → project row → shipped role default → complexity-derived selection → inheritance, validates model and effort independently, and returns compact provenance plus masking/fallback diagnostics. A host-enforced value wins and records the lower choice it masked; an optional malformed, unavailable, or selector-unsupported choice falls back explicitly to inheritance, while a required-but-unhonorable choice stops before dispatch.

After a routed attempt, the parent may submit typed `postAttempt` evidence to the same query. Sufficient work returns terminal `retain`; insufficient work may return one parent-owned `retry` — at the next stable `haiku → sonnet → opus` tier when the model-tier lever applies, and otherwise at the prior attempt's own selection re-resolved through the ordinary precedence chain, with `retry.nextTier` null though a host pin may still change the model — naming only failed bounded-parallel units while retaining successful ones. `retry.escalation` names which lever was pulled. Retries preserve prior routing and validation context, recompute execution shape through the same evidence selector, and stop on invalid provenance, exhaustion, or a tier advance that was requested and defeated by masking, unavailability, or non-advancement. Two total attempts are allowed by default; the sole shipped exception permits `security-auditor` a third attempt only when its signal set is exclusively high-severity review uncertainty, and no policy exceeds three. Children never spawn replacements and successful work is never rerun for reassurance. Routing metadata is operational only — artifact `**Model:**` attribution still records the model that actually authored the artifact.
...
The full-auto single-task runner. It **requires** a registered delivery provider (there is nothing to merge without one) and drives one task all the way to a merged pull request without a human pause: it clears every gate `/wf:run` halts at (following `/wf:run`'s own handoff, so it drives whatever phases the pipeline defines), opens the PR through `/wf:pr`, **waits for the delivery checks to settle — never merging a red one** — converges failing checks through a bounded CI-remediation loop, and finalizes the merge through `/wf:tf`. An orchestrator: the phases and `/wf:tf` own every write, and it writes no artifact at all — it mutates source in exactly one bounded place, the Phase 4.2 CI-remediation loop, where a failing check is distilled in an isolated read-only agent, the minimal fix applied and re-pushed, and the checks re-read, under a 5-iteration bound. Stops honestly with a `SHIP — Blocked` block (naming the missing provider, a failed build phase, unsettled checks, checks the loop could not converge, or a blocked finalize) rather than forcing a partial merge. The review-address loop is **out of scope** — `ship` carries a single marked stub at the point where a future skill-level *slots* mechanism will attach a review step, and drives no reviewer.

**Gates are self-approved, on the record.** A gate is normally cleared by a human, and `ship` clears them without one — so it asks the resolver to issue one **self-approval record per gate** it drives past, binding the gated artifact by digest, and it **halts** at any gate whose approval it cannot then prove (absent, unmatched, not artifact-backed, or approving an artifact that has since changed). The `Gates:` line in the `SHIP —` block reports the tally. A gate whose phase reports no artifact to digest is counted `unapprovable` rather than approved — an honest gap, not a satisfied gate.

**`WF_RUN_MODE` (optional environment variable).** Set on the process that launches the session — **not** in `_local/config.md`, and not by the agent doing the work. It declares whether the run is `unattended` or `attended`; the resolver reads it from its own launch environment, which a dispatched agent cannot mutate, and seals what it observed into every run-evidence record. Unset or unrecognised is recorded as `unestablished`, never guessed in either direction, and is reported as such. It is a label placed out of the running agent's reach, not an authenticated claim: an operator who sets it falsely gets a falsely-labelled record.
...
| Skill | What it does |
|---|---|
| `/wf:research` | One step before `/wf:charter`: grounds an idea in evidence rather than recall. Clarifies the decision and its stakeholders, surfaces the framing's key assumptions, classifies reversibility, and frames 3–4 PICOC research questions behind a plan gate. Gathers the project's own evidence (prior decisions, incidents, metrics) plus external evidence through parallel isolated `wf:research-gatherer` subagents that hunt disconfirming evidence as deliberately as support; grades sources (`T0`–`T3`, `L`) and claims (Strong/Moderate/Weak/Insufficient) and re-fetches every load-bearing citation to verify its quote. Writes `R<NNN>__<slug>/` with `00_brief.md`, `01_findings.md`, and `02_verdict.md` — hard-constraint veto, MADR-shaped options with disconfirming evidence and maturity labels, outside view, confidence, and an eight-row practicality checklist — after an isolated `wf:research-challenger` runs a pre-mortem against the recommendation. On a `Practical` or `Practical — spike first` verdict it seeds `C<NNN>__<slug>/00_intake.md` citing the research, so `Next: /wf:charter <charter-id>` resumes straight at the charter writer. Never runs code; spikes are named, not run. |
