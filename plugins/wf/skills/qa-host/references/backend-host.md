# wf:qa-host — Backend test-host (ephemeral controller wiring)
...
```csharp
// >>> WF-QA-EPHEMERAL ADO-<id> — DO NOT COMMIT — revert: /wf:qa-host api-revert <Service>.<method>
...
```
/wf:qa-host api-revert <target>      # remove the one target's ephemeral block
/wf:qa-host api-revert --all         # sweep every WF-QA-EPHEMERAL block in the repo
```
...
This is the one place `wf:qa-host` edits a **pre-existing** source file (a controller) rather than only new files under a test-host folder. The carve-out is tight:
...
Reminder:    API must rebuild/hot-reload before this route responds.
             ...
             /wf:qa-host api-revert <Service>.<method> (or --all) before committing.
