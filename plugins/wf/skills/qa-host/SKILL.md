---
name: qa-host
description: Scaffolds a routed Angular test-host page for a component that doesn't yet have one — creates AuditTrakker.Web/src/app/code-trakker/<kebab>-test/ with a real-DI component + status-panel template, plus the three standard edits to code-trakker-routing.module.ts (import, child route with auth+entity guards, static-components array). The `augment` mode retrofits type-driven input controls and output observation onto an existing host so a QA scenario can drive inputs the host had pinned. The `api-probe`/`api-revert` modes are the backend analog — they temporarily wire a service/repository method to the most appropriate controller (an ephemeral, sentinel-marked endpoint that's reverted before commit) so a backend QA scenario can exercise it over HTTP, or just resolve the route when the endpoint already exists. Idempotent — re-invoking on an existing host returns its route URL. Use when /wf:qa-auto or /wf:qa-followup needs a runnable URL or endpoint (or a missing control) for a component or service that's still in development.
