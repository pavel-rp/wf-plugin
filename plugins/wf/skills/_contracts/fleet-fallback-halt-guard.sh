...
#
# --selftest runs the same evaluator over seven seeded synthetic chains,
# including the exact pre-fix shape (both trailing steps unconditional) and an
# unconditioned ship:run-initial --gate extend dispatch, and requires the
# evaluator to reject every defective one and accept the sound one.
#
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  selftest_fail=0
  for name in pre-fix halt-clause-incomplete gated-step-damaged row-recording-absent halt-clause-masks-step run-initial-unconditioned; do
  ...
  echo "fleet-fallback-halt-guard: self-test passed — the evaluator rejects the pre-fix, incomplete-halt, damaged-gated-step, unrecorded-halt, halt-clause-masked and run-initial-unconditioned chains and accepts the sound one."
  exit 0
fi
