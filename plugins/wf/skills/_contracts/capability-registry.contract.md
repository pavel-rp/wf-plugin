...
`newest-published-version-read` (added later still, again extending — never
reshaping — the same partitioned surface) is the **newest published version**
read. It answers exactly one question: *"what version does the newest published
state of this workspace declare?"* — the value a consumer would receive on a
fresh install, as opposed to whatever the local working copy declares.

- **It consumes, never derives.** Its inputs are an already-resolved
  `<version-declaration>` (the workspace-relative path of the file that declares
  a unit's version) and an optional `<version-field>` (which value inside that
  declaration holds the version; each owner states its own default when omitted).
  Choosing the declaration, and reading the *local* value to compare against, are
  the caller's job — this operation reads only the published side.
- **It names a version, never where versions are published.** "Published",
  "version", and "declaration" are abstract vocabulary here in the same way
  "branch", "commit", and "check" already are: how a published state is reached
  is entirely the owning capability's business, and no host, marketplace,
  repository, or release-channel noun appears in this contract or in any core
  prose describing the operation.
- **Its result is typed, and it is the second operation on this surface that does
  not follow the silent-empty precedent** (below). A caller comparing a local
  version against this one is making a claim about currency; a bare empty would
  read as "nothing newer exists", i.e. "the installation is current", which is
  precisely the wrong conclusion when the read never happened.

**Degradation shape — a typed degraded result for `review-threads-read` and
`newest-published-version-read`.** The other read-side empties on this surface
(`pr-comments-read`, `checks-read`,
...
backs a merge-blocking claim. Two reads are deliberately **different**.
...
`newest-published-version-read` follows the same typing discipline, one level
down: its result carries a `<read-performed>` flag that is true **only** when a
concrete newest-published version value was actually obtained, and a
`<version>` value that is present **only** then. When `<read-performed>` is
false the result instead carries a `<reason>` drawn from a **closed** token set —
`no-provider` (no capability owns the `delivery` surface), `read-failed` (the
owner's published-side read did not complete), `none-published` (the read
completed but the declaration carries no version in the published state). Unlike
`review-threads-read`, a *performed read with no value* is **not** a valid
success here: there is no useful "the published side genuinely declares nothing"
comparison, so an absent value is `none-published` and never a bare true. The
typing exists for the same reason: a consumer may state that the installation is
current **only** on `<read-performed>` = true, and must otherwise state that the
check did not run and why — a provider-less or failed read is reported as such,
...
"resolved", "reply", "check", "merge",
"activity", "published", "version", and "declaration" are abstract vocabulary,
not hits — a capability's own manifest and
...
- `review-threads-read` also resolves to an empty result, but a **typed** one:
  ...
  empty is explicitly *not* a performed `HEAD_SHA` read-back — one of the two
  reads on this surface that do **not** follow the plain silent-empty precedent,
  exactly
  because it feeds a merge-blocking "no unresolved threads" claim that a silent
  empty must never be allowed to satisfy.
- `newest-published-version-read` is the other typed one, and it has **no**
  plain-directory analogue: a purely local read cannot answer a question about a
  *published* state, so bare-core does **not** substitute one. It resolves to
  `<read-performed>` = false with `<reason>` = `no-provider` and no `<version>` —
  the **stated** provider-less result, never a silent empty. A caller must
  surface that it ran provider-less rather than presenting it as a passed
  currency check; what a caller then falls back to is the caller's own concern,
  not this surface's.
...
- `list_by_status` — enumerate the work items currently in a named workflow
  status. Its return shape is `{id, title, status, updated-at}`; `updated-at` is
  the item's own last-changed moment and is **additive** (WF-525) — every
  pre-existing key keeps its name and meaning. The field is on the
  **enumeration** and deliberately **not** on `get`, whose shape is unchanged: a
  caller that must call `get` to learn whether to call `get` saves nothing,
  because `get` is the call that carries the description.
- `list_statuses` — enumerate the workflow statuses defined for a scope, each
  carrying its name and an abstract `<lifecycle>` value from the closed pair
  `open` | `terminal` (WF-525). It is the surface's one **discovery** read: it
  *answers* a status name rather than consuming one, so a caller can tell
  unfinished work from finished without knowing any status name. **Core never
...
The last five are **read-only query operations**: they enumerate the active
...
**Why `list_statuses` is typed and the other four are not (WF-525).** The other
four answer "which records match this thing you already named?", where an empty
answer is a complete, honest answer. `list_statuses` answers "what does this
tracker *have*?", where an empty answer from a provider that never implemented
the operation is indistinguishable — by emptiness alone — from a provider that
genuinely defines no statuses. The `<operation-supported>` flag is what separates
those two, and the **fourth degradation case** below is what a provider that
omits the operation lands on. That is the same reasoning, and the same shape,
that made `review-threads-read` a typed degraded-empty rather than a silent one.
...
- **Configured, recoverable provider whose fragment omits the operation**
  (WF-525) — the provider resolves fine and every other operation works; it
  simply never bound this one. That is the state **every** tracker pack outside
  this repository is in the day a new operation ships, so it must degrade rather
  than break. The operation resolves to a **typed degraded-empty**: an empty
  result, **silently** — no error and no warning, exactly like the unconfigured
  read — carrying `<operation-supported>` = **false** on every return shape that
  declares that flag (`list_statuses` today). This is a **fourth** case
  *extending* the three above; none of them changes.

  A caller tells the three empties apart **without inferring anything from
  emptiness**, by two mechanisms in a deliberate division of labour:

  1. The **flag on the return** separates the two *configured, recoverable*
     returns from each other — `false` = "could not enumerate", `true` with an
     empty list = a genuine "nothing found".
  2. The **provider record's `state`** — already resolved once per run, before
     any read is issued — separates the *unconfigured* return from both, since
     an unconfigured tracker is known to be unconfigured out-of-band and its
     silent empty carries no flag value at all.

  One boolean carries one value; the third state is a property of the provider
  record, not of the return. An **untyped** empty would have collapsed "could not
  enumerate" into "nothing open", which is precisely the misreport this case
  exists to prevent. A caller that needs a working result on this path takes the
  **explicit lifecycle answer** instead — the install-time question that persists
  the value the operation would otherwise have discovered — so a pack that has
  not implemented the operation still reaches a usable outcome.
...
- **`gate-map:`** — **optional, repeatable.** A capability whose layer emits review labels
  declares each of its gate-eligibility maps with one `gate-map: <rel-path>` line, the path
  forward-slash and **relative to the capability's folder**. The map's shape, its grammar-source
  rule, and what fails are defined in `gate-map.contract.md`; `gate-map-guard.sh` finds every
  declared map through this key alone, naming no capability. A manifest with no `gate-map:` key
  publishes no map (the no-op path); the key changes no resolution, dispatch, or blocking behaviour.
