...
- **`unresolved`** → no body, two distinct diagnoses (a content read is a `local-read` surface → **continue** either way). **Integrity** (an unregistered / dangling-and-unrecoverable capability or plugin root, no declared template, or a resolver-build failure): the matching `resolve_gate` degradation class with a `/wf:resolve` recovery path. **Bad ref** (`ref-not-found` — the root resolved but no file exists at the joined path): fix the ref's shape — it is relative to its root *including any subfolder* (a fragment ref is `fragments/<doc>.md`; the provider record's `fragmentPath` shows the exact shape) — not a registry problem, so refresh only if the pack was genuinely relocated. **Never a wrong-path body, never a raw-read fall-through.**
