...
- **Read side:** `workspace-root-resolve`, `current-branch-query`, `default-base-query`, `last-commit-timestamp-query`, `branch-changes-read`, `pr-comments-read`, `checks-read`, `activity-read`.
