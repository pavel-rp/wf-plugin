...
The verbatim `lite.md` template — the metadata block, `## Objective`, `## Approach`, `## Files`, `## Plan`, and `## Done When` — lives at `lite-template.md`, obtained via the resolver's `resolve_content` (`class: references-template`, `skill: lite`, `ref: lite-template.md`), never a raw `Read` of the plugin-cache path. It is read only on this write path (Phase 4), so it stays out of the boot body. Follow it, then emit it with placeholders substituted.
