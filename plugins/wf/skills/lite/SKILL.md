...
2. **Otherwise**, call `resolve_routing` for role `classify`, execution shape `task`, `supportsModelSelector: true`, and `supportsEffortSelector: false` immediately before dispatch. Emit its compact metadata, stop only on `status: stop`, pass a non-null model selector, and preserve inherited effort when null. Then invoke the **Task** tool with `subagent_type: wf:classify`, passing `{id}` (resolved against `00_reqs.md`). Parse the `CLASSIFY — Complete` block for `Type`, `Confidence`, and `Alternative`. If the classifier returns `CLASSIFY — Error`, stop and surface the reason — do not guess a type inline.
