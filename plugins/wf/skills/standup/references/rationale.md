...
## Phase 4b — why sweeping suppresses the terminus, and why that is acceptable

The sweep moves a task folder to the same destination `/wf:tf` Phase 6 uses, which means a swept task afterwards trips tf's already-archived branch and returns `TF — already-finalized` without running a provider step. The loss is real but bounded, and it is bounded by the sweep set's own precondition. A folder only enters the sweep set when its item was **positively observed this run in a status whose carried lifecycle is `terminal`** — that is, the tracker already reflects the work as finished. The status close tf would perform has therefore already happened upstream, and the only thing genuinely forgone is tf's resolution comment. Trading one comment on an already-closed item for the ability to clear a drifted repository in a single opt-in action is a good trade; trading a *status close* for it would not be, which is exactly why the sweep set is not allowed to widen past positively-observed-terminal.

This is also why the sweep writes no `09_finalize.md`. A sweep is not a finalize: it runs no provider step, posts no comment, and closes nothing. Synthesizing a finalize record for a task that never ran the terminus would put a claim in an artifact that nothing performed — a lie that a later reader, or a later audit, would have no way to detect. The `Swept:` field records what actually happened and stops there.

## Phase 4b — why there is no branch-awareness guard

An obvious-looking safety addition would be to skip a folder whose id matches the currently checked-out branch, on the theory that it might still be live work. It was considered and deliberately dropped. Adding it would require standup to read the current branch, and standup's Allowed clause names exactly one delivery read — `activity-read` — while its Forbidden clause bans naming any version-control tool or command string, which closes the shell route too. So the guard could not be built without widening a non-negotiable section by one operation, for one heuristic.

It is not needed. The positively-observed-terminal precondition already establishes that the folder's work is finished *upstream*, and that fact does not change with whatever branch happens to be checked out — a developer sitting on a branch for an item the tracker has marked terminal is precisely the drift case this feature exists to clear. Skip-if-present covers the remaining overlap with an earlier finalize. Widening a non-negotiable section to add a guard that the existing precondition already subsumes would trade a real invariant for an imagined one.

## Phase 4b — why `--no-write` suppresses the move

The brief settles the flag's spelling and its sweep set but leaves the two-flag combination open. It is resolved toward safety: `--no-write` suppresses the move as well as the briefing artifact.

Two readings were available. The narrow one scopes `--no-write` to the briefing artifact alone, on the grounds that it was introduced to control that file and the sweep has its own opt-in flag. The broad one reads `--no-write` as "change nothing on disk". The narrow reading is defensible on history and would let a user who passed both flags see an irreversible move they plausibly believed they had disabled; the broad reading costs that user one re-run without `--no-write`. Since this is the only irreversible action the skill performs, the asymmetry is decisive — the conservative reading cannot destroy work and the permissive one can. Leaving the combination undefined was not an option: an undefined flag interaction in a runtime-read body is exactly the "plausible-but-wrong next action" the authoring clause-test rules out.
