...
2. **If the branch name contains `/{task-id}-` or `/{numeric-id}-`**, compared case-insensitively (e.g. `feat/{task-id}-…`, `<prefix>/{task-id}-…` in any letter case, or `feature/{numeric-id}-…`) — already on the task branch, continue directly to Phase 0.7.
