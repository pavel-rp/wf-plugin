...
2. **Apply the shared two-arm-plus-collision-guard predicate** (obtain the doc via `resolve_content({ workspaceRoot, ... })`, `class: shared`, `ref: pipeline-conventions.md` — never a raw `Read` of the plugin-cache path) §"Branch gate (bare-core aware)" step 2 — a `/{task-id}-` match (where `{task-id}` differs from `{numeric-id}`) accepted outright, a `/{numeric-id}-`-only match accepted only when the guard's `{task-root}` + `_archive/` folder-scan finds no other folder sharing that numeric run, both compared case-insensitively. Do not restate the predicate here; the shared section is authoritative so the guard cannot drift out of sync. **Accepted** — already on the task branch, continue directly to Phase 0.7.
