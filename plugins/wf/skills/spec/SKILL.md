...
- Resolve the declared `spec.questions` (Phase 2) and `spec.publish` (Phase 4) slots via `resolve_content({ workspaceRoot, ... })` (`class: slot`, `skill: spec`) — **one call per marker** — and, only on a `composed` outcome, follow the served body as prose in this skill's own context. A followed body may perform the operations it names, including writes to the task folder's artifacts.
