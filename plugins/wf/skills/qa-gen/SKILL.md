...
**Backend-only tasks: emit the API baseline instead.** When the task affects no reachable UI route but *does* add or change a callable endpoint/service (the backend-only case from Phase 2), do **not** mark the baseline `[N/A: no runnable UI]`. Emit the single **API baseline** scenario from `references/api-scenarios.md` § API baseline health (same `resolve_content` reference as above) — the primary endpoint responds without a 5xx and accepts the token — and skip the three browser baseline scenarios below. Only fall back to a lone `[N/A: no runnable surface]` scenario when the task has neither a route nor any callable endpoint/service (pure type/config/helper work).
...
Emit the `06_qa.md` output template at `references/qa-template.md` (obtained via the resolver's `resolve_content` — `class: references-template`, `skill: qa-gen`, `ref: qa-template.md` — never a raw `Read` of the plugin-cache path) verbatim. Substitute placeholders. Don't invent extra sections.

---

## Template: `06_qa.md`

The verbatim output template — the coverage-matrix, browser-scenario, API-scenario, capability-scenario, and Baseline-health shapes the run-assistants parse — lives at `references/qa-template.md` (same `resolve_content` reference as above). It is read only on this write path, so it stays out of the boot body. Follow it, then emit it with placeholders substituted.
