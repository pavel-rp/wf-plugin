...
5. **Catalog existing automated coverage.** Look under:
   - `_local/{task-id}/tests/` — a registered unit-test harness's output (e.g. a pure-helper runner).
...
**Host preflight (once per generation).** Before authoring any scenarios, call the bundled resolver once: `resolve_provider({ workspaceRoot, surface: "qa-execution:host" })`. Retain only its `state` as `{host-availability}` for this plan; do not resolve per scenario. `state: ok` means host-dependent scenarios are runnable and carry no availability annotation. `state: unconfigured` or `unrecoverable` means `{host-availability}` is `unavailable`: retain every scenario with either a `Host required:` or `Backend host required:` precondition, add the exact `**Host availability:** unavailable` line defined in `api-scenarios.md`, and render the one plan-level capability gap defined in `qa-template.md`. Do not mark browser/API scenarios without either host requirement unavailable. If the resolver service itself is unavailable, stop and report that the resolver runtime is not loaded — do not author a plan from guessed provider state.
...
| **Manual-browser** | Human or agent observes runtime behavior in the running app | "clicking Save shows a green toast", "dropdown filters list to entries matching the search" | One or more browser scenarios depending on scope. An un-routed target gets `Host required:` and, when `{host-availability}` is `unavailable`, the stable unavailable-host annotation. |
| **API** | The behavior of an endpoint or service method, called over HTTP with a real token | "endpoint returns the widgets for the access level", "repository filters out inactive rows", "POST creates the record and returns 201", "service returns an empty list when none match" | One or more API scenarios. Service-only methods get a `Backend host required:` precondition and, when `{host-availability}` is `unavailable`, the stable unavailable-host annotation. See `api-scenarios.md` (same `resolve_content({ workspaceRoot, ... })` reference as above). |
...
- **Backend-only task (all behavior in backend source, no UI).** Not a stop condition and not a stub. Behavioral criteria classify as **API**; service-only ones get `Backend host required:` and, when the generation preflight found no host owner, `**Host availability:** unavailable`; the Baseline-health suite uses the API baseline. Directly callable endpoint scenarios remain runnable; an all-service-only plan honestly carries the one plan-level capability gap rather than pretending it can run unattended.
