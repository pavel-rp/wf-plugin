...
| --------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `<id>`          | NO       | Task id — whatever shape the active tracker capability produced when the task folder was created (opaque to core), or a local `T<NNN>` id when none was registered. Falls back to inferring from the current branch via `current-branch-query` (the `wf-resolver` `resolve_provider("delivery")` query, first 3+-digit run). First run for a brand-new task needs an explicit id. |
...
- Read-only resolution via `workspace-root-resolve`, `current-branch-query`, and `last-commit-timestamp-query` (the `wf-resolver` `resolve_config` / `resolve_provider("delivery")` queries).
