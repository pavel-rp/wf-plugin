...
Reached only at round ≥2 of a `FAIL`/`PARTIAL` `verify-spec` (Phase 3). Compares **blocking fingerprint sets** — the fingerprints listed under the current `04_verify.md`'s `## Capability findings` heading (round `N`) against the same heading in the most recent `04_verify.history.md` entry (round `N-1`); requirement `FAIL`/`PARTIAL` rows carry no fingerprint and never enter this comparison (they still gate the verdict on their own, per `verify-spec/SKILL.md`).

**Progress test.** At least one round-`N-1` fingerprint absent from round `N`'s set → **progress** → continue to `verify-fix` (cap permitting). Round `N`'s set equal to or a superset of round `N-1`'s (nothing cleared, whether or not something new appeared) → **no progress** → stop **early**, before spending the cap's remaining cycle. Reaching the existing 2-cycle cap with a still-shrinking set also stops, through this same gate — a shrinking set is progress, not an exemption from the cap.
