...
```
/wf:run [<ado-id>] [--auto | --step] [--from <phase>] [--to <phase>] [--no-triage]
```
...
| --------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `<ado-id>`      | NO       | ADO ID, numeric or prefixed. Falls back to inferring from `git branch --show-current` (first 3+-digit run). First run for a brand-new task needs an explicit ID. |
...
Disambiguation: a 3+-digit numeric or `<wi-prefix>-NNN` token is the ID; `--`-prefixed tokens are flags.

---

## Prerequisites

**Before anything else**, read `_local/config.md` for `{task-root}` and `{wi-prefix}`. If it doesn't exist, stop: "Run `/wf:init` first." Never hardcode these values.
...
- Read-only git: `git rev-parse`, `git branch --show-current`, `git status --porcelain`, `git log -1 --format=%cd`.
...
- Run builds, tests, installs, or any git operation that mutates state.
- Execute a phase's logic **inline in your own context** (e.g. do `wf:spec`'s ADO fetch yourself). Inlining defeats the per-phase context isolation that is the whole point. In `--step` you name the command and stop; in `--auto` you dispatch the phase to the `wf:phase-runner` subagent — never run it inline either way.
- **"Rescue" a failed phase subagent by doing its work yourself.** If the `wf:phase-runner` **Task** call returns an error (e.g. it reports a missing tool), do **not** fetch the ADO item / explore the codebase / build the artifact in your own context and feed it into a retry. That re-imports the exact heavy context the isolation exists to keep out, and "wf:run didn't write the file, the subagent did" is not a loophole — it is still inlining. The subagent inherits every tool you have (it declares no `tools:` allowlist), so a genuine tool gap is a bug to fix in the agent, not to route around. Halt and surface (Phase 4); a human or a fixed subagent retries.
...
The QA tail self-orchestrates (`wf:qa-followup` already calls `wf-caps:qa-host` + `wf:qa-auto --only`); `wf:run` sequences into it and stops on the terminal QA verdict.
...
1. Resolve `<ado-id>` (arg or branch inference). If neither, stop: "No ADO ID provided and none could be inferred. Pass it: `/wf:run <ado-id>`."
2. Compute the task folder `{task-root}/{wi-prefix}-{id}/`.
...
**Staleness guard** (borrowed from `wf:qa-followup`): run `git log -1 --format=%cd`; if source commits landed after an artifact was written, warn that downstream artifacts may be stale and offer `--from <phase>` to redo from there.
...
2. **Dispatch the auto-front phase.** Otherwise the next phase is one of `triage` / `spec` / `plan` / `verify-spec` / `qa-gen`. Invoke the **Task** tool with `subagent_type: wf:phase-runner`, passing `phase` (the next-phase token) and `ado-id`. The subagent runs the phase in its own context and returns **only** that phase's Final Output block.
...
The orchestrator's context grows by only one small status block per phase; every heavy read (ADO fetch, codebase exploration, artifact authoring) stays inside the per-phase subagent. **If subagent invocation is unavailable**, the default walk can't dispatch the subagent — announce this and fall back to `--step` behavior (dispatch one phase by name and stop).
