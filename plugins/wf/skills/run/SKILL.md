...
- **`extend`** → resolve the attempt scope `k`: read `{task-root}/{task-id}/05_verify-fix.md` (if present) and every entry in `{task-root}/{task-id}/05_verify-fix.history.md` (if present) for `**Attempt:** <k>` header lines — the same signal `/wf:verify-fix`'s own Phase 1.5 default resolution already reads (WF-663) — and take the highest `k` found. Because this stop is reached only at round ≥2, round 1's unconditional `verify-fix` dispatch has already run at least once, so at least one `**Attempt:**` header is always on disk by this point. The resolved next-phase command is `/wf:verify-fix {task-id} --attempt <k+1>`; carry this exact string forward as the `verify-fix` dispatch command for Phase 3/4 below (both `--auto` and `--step`), for exactly one more cycle, then `verify-spec` again as usual.
