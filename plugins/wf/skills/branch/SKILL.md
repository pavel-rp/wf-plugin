...
- **Already on the task branch** — `BRANCH — already-active`; no new branch is created. The subagent recognises this two ways: the delivery provider's exact-name match on a name it derived itself, and — for a branch whose shape it would not have derived, such as a tracker-prefixed `feat/{task-id}-…` or one whose title has drifted since it was cut — a current-branch check applying the same two-arm-plus-collision-guard predicate as the shared branch gate, compared case-insensitively. A `/{task-id}-` match (where `{task-id}` differs from `{numeric-id}`) is accepted outright; a `/{numeric-id}-`-only match — which also covers the bare-numeric id shape — is accepted only when the guard's scan of `{task-root}` and its `_archive/` finds no other folder sharing that numeric run. The guard is what makes the numeric arm safe, so the check is no longer narrowed away from the shared predicate; the two agree by construction.
