...
- Read-only resolution via `current-branch-query` (the `wf-resolver` `resolve_provider("delivery")` query), used by the Phase 1 branch gate when a delivery provider is registered.
...
3. **`state: ok`** — resolve the current branch via `current-branch-query` (obtain its body via `resolve_content` and follow it).
