...
3. **`state: ok`** — resolve the current branch via `current-branch-query` (obtain its body via `resolve_content({ workspaceRoot, ... })` and follow it).
   - **If the branch name contains `/{task-id}-` or `/{numeric-id}-`**, compared case-insensitively (e.g. `feat/{task-id}-…`, `<prefix>/{task-id}-…` in any letter case, or `feature/{numeric-id}-…`) — already on the task branch, proceed to Phase 2.
