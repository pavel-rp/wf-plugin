...
- **Registered-but-unrecoverable tracker:** a registered capability's manifest can't be read after the install-manifest self-heal — the subagent's tracker `get` (a read) stays silent while the `attach_link` (a write) warns once in the hedged candidate-naming form (naming the unreadable-manifest pack(s) as candidates, never asserting ownership) on the `Body sources:` line, for a net of one warn driven by the write; the body composes local-only with no work-item link and PR creation still proceeds. This tracker residual is independent of the delivery-write `PR — Error` above — both can surface when both surfaces are unrecoverable.
