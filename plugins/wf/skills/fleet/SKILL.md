...
- **Shipper merged without the ceremony block:** the shipper hand-rolled the build/merge with raw `git`/`gh` instead of running `/wf:ship` (or the `/wf:tc`→`/wf:tf` fallback), so its report carries no `SHIP —`/`TF —` block — the OBSERVE proof-of-ceremony check records a ceremony violation in that row's `notes` and surfaces it in REPORT. The merge stands (it cannot be undone), but it is never reported as a clean ceremony run, and the item's skipped gates (verify, QA) are flagged for a follow-up pass.
