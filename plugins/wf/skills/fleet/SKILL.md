...
> - **COMMIT EARLY, PUSH OFTEN.** Cut your branch from the latest delivery base. Push a work-in-progress commit the moment your first file changes, and after every step. Uncommitted progress remains active work: report it with the activation-intent token and a concrete working-state checkpoint so the orchestrator records it in `notes` rather than inferring from silence. WIP commits are fine; a squash-merge erases them.
