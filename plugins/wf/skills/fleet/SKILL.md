...
Invoked with no arguments, `/wf:fleet` **resumes**: it re-reads the scoreboard at `_local/fleet/scoreboard.md`, reconstructs the graph and every `dispatched`, `in-flight`, and `awaiting-confirmation` activation from it, and continues the tick loop. Reconcile each persisted `dispatched` row deterministically before computing readiness: no recorded `agentId`, or conclusive runtime proof that spawn failed/no activation exists, returns it to `queued`; a recorded agent that runtime confirms active becomes `in-flight`; a recorded agent without conclusive live-or-terminal state becomes `awaiting-confirmation` and follows the silence-probe supervision path. Never leave a resumed row stranded in `dispatched`, and never launch replacement work merely because reconciliation is uncertain. If no scoreboard exists, stop and ask for an explicit item set: `/wf:fleet <ids-or-umbrella>`.
...
- Compute the **full ready frontier** in deterministic dependency/input order: queued items whose blockers are all `merged` and that do not share a serialization edge with an `in-flight` or `awaiting-confirmation` item. Resolve each item's model first (the pinned `--model`, otherwise §"Model selection"), then partition the frontier into stable, model-homogeneous groups without reordering items. The full group is only a candidate set; queued excess is never part of a retained routing decision.
- For each model-homogeneous group while pool slots remain, compute available pool slots after counting every `in-flight` and `awaiting-confirmation` activation (both occupy capacity) and take the first `min(group size, available slots, configured pool bound, core execution cap of four)` items as the candidate launch wave. Call `resolve_routing` for exactly that candidate wave, passing its ordered item ids as `unitIds` and its shared model as `invocationModel`. If the returned `effectiveParallelism` is smaller than the candidate wave, dispatch nothing under that decision: narrow to the exact first `effectiveParallelism` items and call `resolve_routing` once more with singleton or multi-item evidence and ordered `unitIds` for that exact launch wave. Retain only the final decision whose `unitCount` and `unitIds` equal the items actually launched. Dispatch every item in that exact wave and leave all excess ready items outside the decision as `queued` for a later fresh initial routing call. `--max-parallel` is the outer pool cap and no decision's `effectiveParallelism` may be exceeded.
- Spawn each selected shipper; after a successful spawn, immediately mark it `in-flight` with its `agentId` and worktree (the `dispatched` state is only a pre-spawn transition and is never left around an active agent). Aggregate completions back in the original dependency/input order, not completion order.
...
For each exact launch wave selected in DISPATCH, call `resolve_routing` immediately before shipper work with `role: "shipper"`, the launch wave's ordered item ids as `unitIds`, and shape evidence selected solely from that launch wave's cardinality. The retained decision's `unitIds` are exactly the units dispatched under it; ready excess remains queued outside the decision:
...
effort. For a bounded-parallel prior, submit the **complete retained launch-wave evaluation** to parent-owned `postAttempt`, including successful siblings as `sufficient` and failed siblings with their insufficiency signals; limit only the fresh replacement Agent dispatch to the resolver-returned `retry.unitIds`. Never omit successes from evaluation or reassurance-rerun them, and restore dependency/input order before scoreboard aggregation.
...
>
> 1. Route with `role: "phase-runner"`, then invoke the initial `/wf:run <ID>` through the Skill tool.
> 2. On every resume, route independently with `role: "phase-runner"` before `/wf:run <ID>`, then invoke it again through the Skill tool.
> 3. On each `RUN — gated` handoff, route with `role: "phase-runner"`, then invoke the exact `/wf:<phase> <ID>` through the Skill tool.
> 4. Route with `role: "pr"`, then invoke `/wf:pr <ID>` through the Skill tool.
> 5. Route with `role: "finalize"`, then invoke `/wf:tf <ID>` through the Skill tool.
>
