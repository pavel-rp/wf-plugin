...
> - **COMMIT EARLY, PUSH OFTEN.** Cut your branch from the latest delivery base — NOT via a harness "branch" skill, whose base switch checks out the trunk and breaks in a worktree. Push a work-in-progress commit the moment your first file changes, and after every step. Uncommitted progress remains active work: report it with the activation-intent token and a concrete working-state checkpoint so the orchestrator records it in `notes` rather than inferring from silence. WIP commits are fine; a squash-merge erases them.
...
>
> 1. Route with `workspaceRoot: <absolute current isolated worktree root from pwd -P>`, `role: "phase-runner"` and `unitIds: ["ship:run-initial"]`, then invoke the initial `/wf:run <ID>` through the Skill tool.
> 2. On every resume, route independently with `workspaceRoot: <absolute current isolated worktree root from pwd -P>`, `role: "phase-runner"` and `unitIds: ["ship:run-resume"]` before `/wf:run <ID>`, then invoke it again through the Skill tool.
> 3. On each `RUN — gated` handoff, route with `workspaceRoot: <absolute current isolated worktree root from pwd -P>`, `role: "phase-runner"` and `unitIds: ["ship:phase"]`, then invoke the exact `/wf:<phase> <ID>` through the Skill tool.
> 4. Route with `workspaceRoot: <absolute current isolated worktree root from pwd -P>`, `role: "pr"` and `unitIds: ["ship:pr"]`, then invoke `/wf:pr <ID>` through the Skill tool.
> 5. Route with `workspaceRoot: <absolute current isolated worktree root from pwd -P>`, `role: "finalize"` and `unitIds: ["ship:finalize"]`, then invoke `/wf:tf <ID>` through the Skill tool.
>
