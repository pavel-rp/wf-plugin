...
- Compute the **full ready frontier** in deterministic dependency/input order: queued items whose blockers are all `merged` and that do not share a serialization edge with an in-flight item. Resolve each item's model first (the pinned `--model`, otherwise §"Model selection"), then partition the frontier into stable, model-homogeneous waves without reordering items. Do not truncate it to `--max-parallel` before routing.
- For each model-homogeneous wave while pool slots remain, call `resolve_routing` exactly as specified by §"The shipper dispatch template", passing that wave's shared model as `invocationModel`. On a dispatch decision, compute the available pool slots, then take only the first `min(available slots, effectiveParallelism)` items from that wave. Dispatch that subset and leave every excess ready item `queued` for a later tick; `--max-parallel` is an outer pool cap, while each decision's `effectiveParallelism` is its wave cap and neither may be exceeded.
- Spawn each selected shipper, then mark it `dispatched` with its `agentId` and worktree. Aggregate completions back in the original dependency/input order, not completion order.
...
>
> **EXECUTE — THE CEREMONY IS MANDATORY, NOT ONE OPTION AMONG SEVERAL.** Route the initial edge with `role: "shipper"`, then drive the merge **through `/wf:ship <ID>`**, invoked with the Skill tool. This is the required path: the point of the pipeline is to run the gated wf build chain (spec → plan → tasks → implement → verify → qa), not merely to produce a merge. **Hand-driving the build and merge yourself with raw `git`/`gh` is NOT an acceptable substitute for the skill** — a green pull request reached by hand skips every gate the ceremony exists to enforce and does not count as done. Only if that Skill is genuinely unavailable or its checks loop cannot run, fall back to the project pipeline: route with `role: "phase-runner"` before the initial `/wf:run <ID>`; on every resume route independently with `role: "phase-runner"` before `/wf:run <ID>`; on each `RUN — gated` handoff route with `role: "phase-runner"` before the exact `/wf:<phase> <ID>`; then route with `role: "pr"` before `/wf:pr <ID>`, and route with `role: "finalize"` before `/wf:tf <ID>`. Each fallback edge gets its own decision and compact record immediately before execution.
>
...
**C. Genuine stall → recycle, then replace.** *Only* when the runtime shows no child activity across TWO consecutive ticks, the delivery reads show **no** branch/pull-request artifact, and no explicit dirty working-state checkpoint was reported — i.e. truly zero observable artifacts (the advisor-loop pattern). Silence never proves a clean or dirty tree, so do not raw-query the child's worktree. Before each recycled activation, call `resolve_routing` again for `role: "shipper"` with the one-item recovery evidence, the retained prior attempt, and the same `invocationModel` selected for that item; emit its compact record and hard-stop that recovery on stop/diagnostic. Only the fleet parent evaluates sufficiency and submits `postAttempt`. On dispatch, `TaskStop` the agent, then `SendMessage` the same id: "Recycled for zero artifacts. Skip ALL remaining setup. FIRST action: start the pipeline NOW (`/wf:ship <ID>`). Commit early, push often. No advisor/planning prose." If a resumed instance also produces nothing across two more ticks, retain the failed attempt evidence, `TaskStop` for good, resolve the fresh replacement independently through parent-owned `postAttempt` with that item's `invocationModel`, and dispatch a **fresh** agent with the full template. **Before recycling, always confirm the agent's ACTUAL worktree from the runtime's live agent state** — an agent that migrated off its assigned path looks stalled but isn't.

**D. Mechanical-tail stall → take over by re-arming, not by hand.** Work is committed and clean but the agent idles before the pull request / merge. The orchestrator issues **no** raw delivery command of its own, so take over by re-arming the shipper: resolve a one-item `role: "shipper"` recovery attempt first with the item's retained `invocationModel`, then `SendMessage` it to run its finalize step now; or, if it is truly dead, `TaskStop` it, submit the retained insufficiency through fleet-owned `postAttempt`, resolve the authorized replacement with the same or deliberately upgraded per-item `invocationModel`, and dispatch a **fresh** `/wf:ship <ID>` shipper on the same id — its committed work is durable on the branch and `/wf:ship` resumes detect-first (it never double-merges). Take over only when the item is genuinely done bar the delivery tail.
