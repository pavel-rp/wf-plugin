...
- Read an item's run-evidence receipts read-only via the `wf-resolver` `read_run_evidence({ workspaceRoot, taskId })` query — the **only** accepted proof that an item's ceremony ran. The orchestrator never records a receipt, and never reaches the receipt destination directly.
...
**The ceremony verdict is one such state.** Every item that reaches a terminal outcome carries exactly one of `proven: <artifact-backed | invocation-only> — <the matched subjects>` or `unproven: <the stated reason>` in its `notes`, in that same rendering, appended rather than overwriting. It is recorded **alongside** the delivery outcome, never instead of it: the `status` cell keeps its enumerated value, so a merged item whose ceremony could not be proven is `merged` in `status` and `unproven: …` in `notes`. An item with no verdict yet simply has no such entry — the row itself is never absent.
...
- **Proof-of-ceremony check.** When a shipper reports its item merged, **do not read its report for proof.** A reported terminal block is composed by the agent whose compliance it attests to, so its presence is satisfied by typing and proves nothing. Resolve the item's own receipts instead: call the `wf-resolver` `read_run_evidence({ workspaceRoot, taskId })` query **once per item, at the point the verdict is computed**, reusing the Prerequisites `workspaceRoot` and passing the item's id verbatim as `taskId`. Then apply the match rule:
  - **Only `matched` records are proof.** A record the resolver could not admit comes back under `unmatched` with its own stated `reason` — record that reason, never count it. A shipper that writes a receipt-shaped file itself lands there, so a self-authored receipt is never proof.
  - **Window on `issuedAt`.** A receipt is scoped to the **task**, not the run, and carries no per-run nonce, so a matched record counts only when its `issuedAt` falls at or after this run's `**Opened:**` stamp. An earlier one belongs to a previous run of the same item and is recorded `stale-window`.
  - **The route fixes the required set.** The two delivery-ceremony subjects (`ship`, `tf`) are required on **every** route — no route excuses either. The route narrows only the five spine subjects (`spec`, `plan`, `implement`, `verify-spec`, `qa-gen`), and is read from the in-window matched set's own shape, never from anything the shipper reports: **all five matched** → the full-chain route; **none matched** → the pipeline driver routed the item to its single-pass phase, which files no spine receipt, so the two ceremony receipts are that item's complete proof set and the five absent subjects count against nothing; **some but not all** → the route is not decidable from the evidence, recorded as its own stated reason rather than guessed in either direction.
  - **Weight the two receipt classes apart.** Every matched record carries an `evidenceClass`. The spine subjects are `artifact-backed` — the resolver digested an artifact of its own accord. The two ceremony subjects are `invocation-only`: they attest only that the skill reached its completion point and invoked the resolver there, which is strictly weaker than that the ceremony *succeeded*, and they are filed on every terminal rather than only the clean one. Tally the two classes apart and never sum them into one figure; an `invocation-only` record never satisfies a required spine subject on the full-chain route.
  - **Everything else is unproven.** A read status other than `ok`, a missing required subject, a stale-window record, an undecidable route, or a non-null `diagnostic` each resolve to **unproven**, carrying the mechanical fact that produced it. The check **fails closed**: an item is proven by evidence alone, never by the absence of a contrary signal.

  Record the verdict in that row's `notes` per the **per-item row convention**, and carry the run's counts into the tick REPORT line. **Do not assert the cause** — record the read status, the missing subjects, and any `unmatched` reason, and never claim *why* they are missing. A merge cannot be undone, so this check is preventive (the dispatch prompt makes the ceremony mandatory) and detective (this surfacing): its value is flagging an unproven item so its possibly-skipped gates can be run as a follow-up, and never logging one as a clean run.
