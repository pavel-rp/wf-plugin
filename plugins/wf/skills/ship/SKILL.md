...
| `--gate extend`   | NO       | Resumes a headless run whose `verify-fix` loop previously halted at its stop gate (`run/SKILL.md` §"The verify⇄fix stop gate") instead of starting from `triage`. Admitted only when §"The `--gate extend` admission check" finds a recorded loop stop in the task folder; forwarded once, on ship's own **entry** `/wf:run` invocation only (§"Phase 2: Drive the build chain past the gates") — never on a resume re-invocation. |
...
Every executable sibling-Skill edge below is routed independently and immediately before that Skill-tool call. Call `resolve_routing` with `workspaceRoot: <absolute pwd -P workspace root>`, the edge's stated stable `role`, exactly one canonical stable `unitIds` entry for that edge (`ship:branch`, `ship:run-initial`, `ship:phase`, `ship:run-resume`, `ship:ci-commit`, `ship:pr`, or `ship:finalize`), the edge's own `shapeEvidence` from the per-edge rule below, `supportsModelSelector: true`, and `supportsEffortSelector: true`. Include `actualModel` only when the host exposes it. Emit the compact operational record, separate from artifact attribution. Hard-stop before the Skill call on `status: stop` or non-null `diagnostic`; otherwise obey `executionShape` exactly (this evidence selects `inline`). The `ship` parent evaluates the returned terminal block; only it may submit `postAttempt` for a contract-defined insufficiency and retry that edge. A child never invokes its own replacement.

**The evidence is per edge, not one literal reused for all of them.** Every edge passes `workSurface: "caller-context"`, `atomicity: "atomic"`, `unitCount: 1`, `unitsIndependent: false`, `ambiguity: "none"`, `risk: "low"`, `toolWork: "none"`, `validation: "mechanical"`, `contextIsolation: "none"`, `independentReview: false`, `requestedParallelism: 1`, and states its **own** `returnContract` — the one dimension that varies across these edges:

- `returnContract: "judgment"` — `ship:phase` and `ship:pr`, whose returned block is weighed as a judgment about the work it reports on.
- `returnContract: "mechanically-judgeable"` — `ship:branch`, `ship:run-initial`, `ship:run-resume`, `ship:ci-commit`, and `ship:finalize`, whose blocks are settled by a token match.

Every other dimension is held constant **deliberately, not lazily**. Seven of them — `workSurface`, `ambiguity`, `risk`, `toolWork`, `validation`, `contextIsolation`, `independentReview` — feed the resolver's isolation predicate, so varying one would move the edge off `inline` and turn a Skill invocation into a dispatch, a different execution model than this section describes. The remaining three — `atomicity`, `unitsIndependent`, `requestedParallelism` — are pinned by the single-unit contract every edge here routes under. `returnContract` is the only evidence dimension that varies a selection without moving the shape.

**What an `inline` edge does with a returned selection.** It runs the sibling skill in this skill's own context, so it hands the selection to no child directly and passes no selector: it records the returned `model` and `effort` with each one's `source` in the compact operational record. The selection is still load-bearing — it is the provenance a downstream dispatch inherits and reports, and a value returned `masked` or carrying a non-null `fallback` is recorded as exactly that rather than silently dropped.
...
## The `--gate extend` admission check

Runs once, immediately after Phase 1 resolves `{task-id}` and before Phase 2's loop begins, **only** when `--gate extend` was passed on this invocation. It decides whether the flag is honored on Phase 2 step 2's entry invocation — never whether the run itself proceeds; an admitted-false result blocks here, before any phase edge, rather than falling through to an unanswered gate downstream. It never admits on a round-plus-attempt-count heuristic alone — it consults the strongest available recorded evidence of the actual stop, and when none exists, reconstructs the real stop predicate `run/SKILL.md`'s own verify⇄fix stop gate applies, cross-checked against the loop it is about to resume.

1. **Read the task folder's own persisted artifacts** — `{task-root}/{task-id}/04_verify.md`, `04_verify.history.md`, `05_verify-fix.md`, and `05_verify-fix.history.md` — exactly as they stand (a resumed attempt reads whatever `/wf:fleet` seeded into this worktree; a same-worktree resume reads its own prior state). `ship` only reads here; it authors nothing in any of them.
2. **Compute round `N` and loop identity `L`** per `finding-ledger.md` §"Round-number derivation" / §"Loop identity" (`resolve_content({ workspaceRoot, ... })`, `class: references-template`, `skill: verify-spec`, `ref: finding-ledger.md`) — `N` read off `04_verify.md`'s `**Round:**` line, `L` the count of `04_verify.history.md` entries carrying `**Verdict:** PASS` — never recomputed independently here. `04_verify.md` absent, or `N < 2` → **not admitted** (step 6).
3. **Recorded-choice path (primary).** Call `read_run_evidence({ workspaceRoot, taskId })`. `status: unsupported`, `status: malformed` (a whole-ledger parse failure — a distinct read outcome from `unsupported`, and arguably the strongest tamper/corruption signal of the three non-`ok`/`absent` outcomes), or the resolver unavailable is **not** "no recorded choice" — it is the same evidence-tools-unavailable class Edge Cases already halts on: **not admitted** (step 6), never silently treated as absent so the run falls through to the heuristic fallback below on an evidence lookup that never actually completed. On a `status: ok` or `status: absent` read (no ledger yet — a legitimate no-record state, not a failure), apply two fail-closed checks **before** looking for a match — either one alone is enough to stop here, never reaching step 4:
   - **`status: ok` and `unreadableRecords > 0`** — the ledger carries at least one record too malformed to read at all. A read that cannot vouch for every record it holds cannot be trusted to have found every relevant one either → **not admitted** (step 6).
   - **The exact-subject record `verify-loop:l<L>:r<N>:extend` is present, but only inside `unmatched`** — a proven-invalid or tampered record per its `reason` taxonomy (`seal-mismatch`, `record-inadmissible`, etc.), never inside `matched` → **not admitted** (step 6). A record this ledger itself could not vouch for is not evidence of a genuine stop.

   Only a clean read — `status: ok`/`status: absent`, zero `unreadableRecords`, and the exact-subject record either absent entirely or found in `matched` — may fall through to the ordinary match check: look for a `matched` entry `kind: gate-approval`, `subject: verify-loop:l<L>:r<N>:extend` — the same evidence class and subject shape `run/SKILL.md`'s stop gate itself writes and reads. Found → **admitted**, skip to step 7: this is the strongest available evidence, since the resolver already sealed this exact loop/round's `extend` choice. Not found → continue to step 4 — this is the only path that reaches the fallback, and it requires a completed, trustworthy read with no unreadable or tampered record standing in for "no match."
4. **No recorded choice — reconstruct the actual stop predicate (fallback).** Take round `N`'s blocking-fingerprint set and `<passed>/<total>` count from `04_verify.md`. Identify round `N-1`'s same data via `finding-ledger.md` §"Round-number derivation", applying its **"De-duplicating a resumed rotation"** step first — collapsing any adjacent `04_verify.md`/`.history.md` pair sharing an identical `**Commit:**` and `**Audited at:**` pair into one entry — before taking the (now-deduplicated) most recent `.history.md` entry as round `N-1`. **Never** take the raw, undeduplicated topmost `.history.md` entry directly: a run resumed mid-rotation can leave it identical to the current `04_verify.md` report, which would compare round `N` against itself and always register "no progress" regardless of the loop's real history. Apply the identical Progress test `run/SKILL.md`'s verify⇄fix stop gate applies: **progress** (a round-`N-1` fingerprint absent from round `N`'s set, or `<passed>` increased) → **not admitted** (step 6) — this round is a live continuation, not a halted one. **No progress** (round `N`'s set equal to or a superset of round `N-1`'s, and `<passed>` did not increase) → continue to step 5.
5. **Loop-identity cross-check.** `05_verify-fix.md` (or `.history.md`) must carry an attempt record at its highest `**Attempt:** <k>` header (WF-569's `--attempt <k>` scope) whose fingerprints (`attempt-ledger.md` §"Fingerprint identity", `resolve_content({ workspaceRoot, ... })`, `class: references-template`, `skill: verify-fix`, `ref: attempt-ledger.md`) intersect round `N-1`'s blocking-fingerprint set from step 4 — confirming the attempt record actually worked this loop's prior-round findings, not a stale record left over from a different, already-finished loop. Intersecting record found → **admitted**, continue to step 7; none found → **not admitted** (step 6).
6. **Not admitted** — `04_verify.md` absent or round `N < 2` (step 2), the recorded-choice read's evidence tools are unavailable or report `status: malformed` (step 3), the read came back `unreadableRecords > 0` (step 3), the exact-subject record was found only in `unmatched` (step 3), or a completed clean read found no recorded choice and either the no-progress predicate fails (step 4) or no intersecting attempt record is found (step 5) → `SHIP — Blocked`, reason: "`--gate extend` admission check found no recorded loop stop in the task folder — nothing to resume" (for the evidence-tools-unavailable or `malformed`-ledger case, the stated unavailability/corruption reason, mirroring Edge Cases' "The evidence tools are unavailable"; for the unreadable-ledger case, "the evidence ledger reports N unreadable record(s) — cannot trust it found no match"; for the tampered-record case, "the recorded `extend` choice failed the ledger's own seal check — treated as no recorded choice, fail-closed"). `Next: /wf:ship <id>` (without `--gate extend`, once a real hand-off exists, once the evidence tools are restored, or once the ledger's unreadable/tampered/malformed state is resolved). No phase edge is dispatched.
7. **Admitted** (step 3 or step 5) → hold `--gate extend` for Phase 2 step 2's entry invocation.
...
2. **Loop the pipeline driver.** First re-derive the gate tally detect-first: call `read_run_evidence({ workspaceRoot, taskId: {task-id} })` once and seed the `Gates:` counts from the approvals already durable for this task, so a resumed run does not under-report gates an earlier segment cleared. Then route the initial edge with `workspaceRoot: <absolute pwd -P workspace root>`, `role: "phase-runner"` and `unitIds: ["ship:run-initial"]`, then invoke `/wf:run <id> --headless` through the Skill tool — **`--headless --gate extend`** on this one entry invocation when, and only when, §"The `--gate extend` admission check" admitted it this run; every later loop re-invocation (the resume edge below) passes `--headless` alone, so the answer binds to the stop present at entry and a later stop in the resumed attempt is a fresh halt, not one this run already spent — and read its `RUN —` block. `--headless` is what makes an unanswerable verify⇄fix stop gate (its own §"The verify⇄fix stop gate") a clean `RUN — blocked` instead of a wait no one here can satisfy:
   - **Gated** (`RUN — gated`, whose `Run next:` field names a phase command) → route the exact dynamic phase edge with `workspaceRoot: <absolute pwd -P workspace root>`, `role: "phase-runner"` and `unitIds: ["ship:phase"]`, invoke `Run next:`'s command **verbatim** — the literal string it printed, including any trailing flags it carries (e.g. a `verify-fix` reached through the verify⇄fix stop gate's `extend` outcome prints `--attempt <k+1>`; `ship` never drops or reconstructs it as a bare `/wf:<phase> <id>`) — through the Skill tool, then **clear that gate under §"Approve every gate this run drives past"** before going further, and only then route the resume edge independently with `workspaceRoot: <absolute pwd -P workspace root>`, `role: "phase-runner"` and `unitIds: ["ship:run-resume"]` and **re-invoke `/wf:run <id> --headless`** (never `--gate extend` again this run). This is the unattended equivalent of a human clearing the gate — and the approval is what makes it evidence rather than assertion. **[ceiling checkpoint]** After each gated phase clears and before the next loop iteration, apply the context-ceiling checkpoint (§"Context ceiling checkpoint").
   - **Complete** (`RUN — complete` / ready for review) → the SDD build chain is done; continue to Phase 3.
   - **Blocked at the verify⇄fix stop gate** (`RUN — blocked` whose `Gate:` line names the verify-loop stop, per `run/SKILL.md` §"The verify⇄fix stop gate") → `SHIP — Blocked`, reason **`verify-loop-halt`** — this exact token, distinct from every other blocked/error reason below, naming nothing further of the phase's own text. `04_verify.md`/`05_verify-fix.md` are left exactly as `/wf:run` left them; `ship` authors nothing there. Stop.
   - **Blocked / error, any other reason** (`RUN — blocked` for any other cause, `RUN — error`, or a driven phase returns its own `… — Error`, or a driven phase halts awaiting human input) → `SHIP — Blocked`, surface the phase's reason, stop. Do not attempt to complete a failed phase yourself.
...
- **The `verify-fix` loop halts at its stop gate:** Phase 2 stops with `SHIP — Blocked`, reason `verify-loop-halt` — a distinct, stable token (never the phase's raw halt text) so a caller can recognize this specific class and, when authorized, retry with `--gate extend`. `04_verify.md`/`05_verify-fix.md` keep the ledger and attempt records intact; `ship` never writes either.
- **`--gate extend` with no recorded loop stop to resume:** the admission check finds no recorded `gate-approval` for this loop/round, and either round `N < 2`, the reconstructed Progress test shows progress (not a genuine halt), or no attempt record in `05_verify-fix.md`/`.history.md` fingerprint-intersects round `N-1`'s blocking set → `SHIP — Blocked` naming the missing hand-off, before any phase edge — never a silent fall-through to an unanswered gate downstream, and never an admission on the round/attempt-count heuristic alone.
- **`--gate extend`'s recorded-choice read cannot complete:** `read_run_evidence`'s evidence tools are unavailable or the ledger is unsupported → `SHIP — Blocked` naming that unavailability, the same fail-closed class as "The evidence tools are unavailable" below — never treated as "no recorded choice" and silently passed through to the heuristic fallback on a read that never actually completed.
- **`--gate extend`'s recorded-choice read completes but the ledger is unreadable or the exact-subject record is tampered:** a `status: ok` read reporting `unreadableRecords > 0`, or one whose exact-subject `verify-loop:l<L>:r<N>:extend` record appears only in `unmatched` (a proven-invalid/tampered record per its `reason` taxonomy), fails closed at step 3 → `SHIP — Blocked` naming the unreadable-ledger or tampered-record reason — never falls through to step 4's heuristic fallback and admits the gate on process signals alone. Distinct from the evidence-tools-unavailable case above: here the read itself succeeded, but what it found could not be trusted as "no match."
- **`--gate extend`'s recorded-choice read reports `status: malformed`:** a whole-ledger parse failure — a third, distinct outcome from `unsupported`, and arguably the strongest corruption signal of the three — fails closed at step 3 exactly like the evidence-tools-unavailable case, never treated as "no recorded choice" and never falling through to step 4's heuristic fallback.
