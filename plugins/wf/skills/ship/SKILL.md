...
Runs once, immediately after Phase 1 resolves `{task-id}` and before Phase 2's loop begins, **only** when `--gate extend` was passed on this invocation. It decides whether the flag is honored on Phase 2 step 2's entry invocation — never whether the run itself proceeds; an admitted-false result blocks here, before any phase edge, rather than falling through to an unanswered gate downstream.

1. **Read the task folder's own persisted artifacts** — `{task-root}/{task-id}/04_verify.md` and `05_verify-fix.md` — exactly as they stand (a resumed attempt reads whatever `/wf:fleet` seeded into this worktree; a same-worktree resume reads its own prior state). `ship` only reads here; it authors nothing in either file.
2. **Admitted** requires **both**: `04_verify.md`'s `**Round:**` line reads ≥2 with a non-empty blocking set recorded under it (the same loop-identity/round reckoning `run/SKILL.md` §"The verify⇄fix stop gate" uses, via `finding-ledger.md` §"Loop identity" — never recomputed here, only read), **and** `05_verify-fix.md` carries an attempt record for that same loop at its highest attempt `k` (WF-569's `--attempt <k>` scope). Both present → hold `--gate extend` for Phase 2 step 2's entry invocation.
3. **Not admitted** — either file absent, or present without the round/attempt shape above → `SHIP — Blocked`, reason: "`--gate extend` admission check found no recorded loop stop in the task folder — nothing to resume." `Next: /wf:ship <id>` (without `--gate extend`, once a real hand-off exists). No phase edge is dispatched.
...
- **`--gate extend` with no recorded loop stop to resume:** the admission check finds no round-≥2 blocking set in `04_verify.md` and/or no matching attempt record in `05_verify-fix.md` → `SHIP — Blocked` naming the missing hand-off, before any phase edge — never a silent fall-through to an unanswered gate downstream.
