...
1. **Ensure the task branch.** Invoke `/wf:branch <id>` through the Skill tool so all subsequent source lands on the task branch (idempotent — `BRANCH — already-active` when already on it). On `BRANCH — Error` (e.g. a dirty tree that blocks the base switch) → `SHIP — Blocked`, surface the reason, stop.

2. **Loop the pipeline driver.** Invoke `/wf:run <id>` through the Skill tool and read its `RUN —` block:
   - **Gated** (`RUN — gated`, with a `Run next: /wf:<phase> <id>` line) → invoke that exact `/wf:<phase> <id>` command through the Skill tool, then **re-invoke `/wf:run <id>`**. This is the unattended equivalent of a human clearing the gate.
...
Invoke `/wf:tf <id>` (forwarding `--status <name>` when passed) through the Skill tool. `/wf:tf` merges the pull request through the delivery provider's `pr-merge` operation (detect-first — never a double-merge), posts the resolution comment and closes the work item through the tracker provider when one is registered, then archives the task folder and updates the index locally. Read its `TF —` block:
