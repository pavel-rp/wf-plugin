...
6. **Not admitted** — `04_verify.md` absent or round `N < 2` (step 2), the recorded-choice read's evidence tools are unavailable (step 3), the read came back `unreadableRecords > 0` (step 3), the exact-subject record was found only in `unmatched` (step 3), or a completed clean read found no recorded choice and either the no-progress predicate fails (step 4) or no intersecting attempt record is found (step 5) → `SHIP — Blocked`, reason: "`--gate extend` admission check found no recorded loop stop in the task folder — nothing to resume" (for the evidence-tools-unavailable case, the stated unavailability reason, mirroring Edge Cases' "The evidence tools are unavailable"; for the unreadable-ledger case, "the evidence ledger reports N unreadable record(s) — cannot trust it found no match"; for the tampered-record case, "the recorded `extend` choice failed the ledger's own seal check — treated as no recorded choice, fail-closed"). `Next: /wf:ship <id>` (without `--gate extend`, once a real hand-off exists, once the evidence tools are restored, or once the ledger's unreadable/tampered record is resolved). No phase edge is dispatched.
