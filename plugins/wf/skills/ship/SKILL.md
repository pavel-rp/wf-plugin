...
4. **No recorded choice — reconstruct the actual stop predicate (fallback).** Take round `N`'s blocking-fingerprint set and `<passed>/<total>` count from `04_verify.md`, and round `N-1`'s same data from the most recent `04_verify.history.md` entry. Apply the identical Progress test `run/SKILL.md`'s verify⇄fix stop gate applies: **progress** (a round-`N-1` fingerprint absent from round `N`'s set, or `<passed>` increased) → **not admitted** (step 6) — this round is a live continuation, not a halted one. **No progress** (round `N`'s set equal to or a superset of round `N-1`'s, and `<passed>` did not increase) → continue to step 5.
