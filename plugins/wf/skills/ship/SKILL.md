...
3. **Recorded-choice path (primary).** Call `read_run_evidence({ workspaceRoot, taskId })` and look for a `matched` entry `kind: gate-approval`, `subject: verify-loop:l<L>:r<N>:extend` — the same evidence class and subject shape `run/SKILL.md`'s stop gate itself writes and reads. Found → **admitted**, skip to step 7: this is the strongest available evidence, since the resolver already sealed this exact loop/round's `extend` choice.
...
6. **Not admitted** — `04_verify.md` absent or round `N < 2` (step 2), or no recorded choice and either the no-progress predicate fails (step 4) or no intersecting attempt record is found (step 5) → `SHIP — Blocked`, reason: "`--gate extend` admission check found no recorded loop stop in the task folder — nothing to resume." `Next: /wf:ship <id>` (without `--gate extend`, once a real hand-off exists). No phase edge is dispatched.
