...
3. **Recorded-choice path (primary).** Call `read_run_evidence({ workspaceRoot, taskId })`. `status: unsupported` or the resolver unavailable is **not** "no recorded choice" — it is the same evidence-tools-unavailable class Edge Cases already halts on: **not admitted** (step 6), never silently treated as absent so the run falls through to the heuristic fallback below on an evidence lookup that never actually completed. `status: ok` or `status: absent` (no ledger yet — a legitimate no-record state, not a failure) — look for a `matched` entry `kind: gate-approval`, `subject: verify-loop:l<L>:r<N>:extend` — the same evidence class and subject shape `run/SKILL.md`'s stop gate itself writes and reads. Found → **admitted**, skip to step 7: this is the strongest available evidence, since the resolver already sealed this exact loop/round's `extend` choice. Not found → continue to step 4 — this is the only path that reaches the fallback, and it requires a completed, trustworthy read.
...
6. **Not admitted** — `04_verify.md` absent or round `N < 2` (step 2), the recorded-choice read's evidence tools are unavailable (step 3), or a completed read found no recorded choice and either the no-progress predicate fails (step 4) or no intersecting attempt record is found (step 5) → `SHIP — Blocked`, reason: "`--gate extend` admission check found no recorded loop stop in the task folder — nothing to resume" (or, for the evidence-tools-unavailable case, the stated unavailability reason, mirroring Edge Cases' "The evidence tools are unavailable"). `Next: /wf:ship <id>` (without `--gate extend`, once a real hand-off exists, or once the evidence tools are restored). No phase edge is dispatched.
