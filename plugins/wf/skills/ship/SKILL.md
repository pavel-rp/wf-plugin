...
Every other dimension is held constant **deliberately, not lazily**: each one feeds the resolver's isolation predicate, so varying it would move the edge off `inline` and turn a Skill invocation into a dispatch — a different execution model than this section describes. `returnContract` is the only evidence dimension that varies a selection here without moving the shape.
