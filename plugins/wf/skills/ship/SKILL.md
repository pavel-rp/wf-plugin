...
1. **Ensure the task branch.** Route this edge with `role: "branch"` under §"Fixed sibling-Skill routing", then invoke `/wf:branch <id>` through the Skill tool so all subsequent source lands on the task branch (idempotent — `BRANCH — already-active` when already on it). On `BRANCH — Error`, surface the reason and stop. A dirty working state is preserved across the delivery provider's capture/switch/reapply operation; it is carried progress, not by itself a branch error. Only an actual carry conflict or provider error blocks shipping.

2. **Loop the pipeline driver.** Route the initial edge with `role: "phase-runner"`, then invoke `/wf:run <id>` through the Skill tool and read its `RUN —` block:
   - **Gated** (`RUN — gated`, whose `Run next:` field names a phase command) → route the exact dynamic phase edge with `role: "phase-runner"`, invoke that exact `/wf:<phase> <id>` command through the Skill tool, then route the resume edge independently with `role: "phase-runner"` and **re-invoke `/wf:run <id>`**. This is the unattended equivalent of a human clearing the gate.
...
1. **Open it.** Route this edge with `role: "pr"` under §"Fixed sibling-Skill routing", then invoke `/wf:pr <id>` through the Skill tool — it commits and pushes any pending work and opens the pull request through the delivery provider. Read its `PR —` block; on its error state → `SHIP — Blocked`, surface the reason, stop.
