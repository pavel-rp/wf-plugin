...
4. **Blocking findings** (round 1: CRITICAL/HIGH routed to `charter-writer` or `decomposer`; round ≥2: `blocking: yes` — CRITICAL anywhere, HIGH on a section the reviewer's snapshot diff marked changed, or the OUT-4 size-budget-overrun checklist row (SUB-3), which blocks in every round irrespective of the changed-text rule):
   ...
   - **Round cap:** Check `03_review-log.md`'s `## Cap-gate decisions` first — each row has the mechanical, fully-labeled form `- Round <N> | revision: <M> of <cap> | choice: extend|accept|stop | status: pending|applied` (mirroring `## Growth authorizations`'s labeled-field shape), `<M> of <cap>` frozen at the values read when the row was created. A `status: pending` row, or any `choice: stop` row regardless of status, needs resolving now — skip straight to its branch below. Otherwise, if `Revisions used` has reached the header's `<cap>` (starts at 3; only ever raised by an extend) with blocking findings left, this is a fresh cap hit: **headless** stops `CHARTER — Blocked` (max rounds), residual findings listed, no gate, exactly as before; **interactive** asks once via `AskUserQuestion` (*extend by one revision* / *accept the residual as warnings* / *stop*) and appends the choice as a new `status: pending` row (create the section on first use) *before* acting, then proceeds to its branch below. Every branch acts on the blocking findings already recorded under this cap-hit round's own `## Round <N>` heading — never a fresh review to re-derive them — and marks the row `status: applied` only once its entire outcome below is reached, never earlier. **One re-entrancy rule governs every branch:** each step below names the on-disk fact its own completion leaves behind; perform a step only while that fact is absent, skip it once present, and mark `applied` when the last step's fact is present — so one recorded choice yields one outcome whether the branch runs straight through or a resume lands mid-sequence. `applied` therefore means every fact is on disk and no step remains (a `stop` row is still re-entered, but only to re-emit a terminal block that mutates nothing); `pending` means re-enter here and let the facts decide. **extend** — raise `<cap>` by 1 in place (fact: the header's live `<cap>` exceeds the row's frozen `<cap>`; the grepped `M of cap` header shape unchanged — C020's `11 of 11 (8 user-authorized extensions)` is the prior art), then fall through to "Otherwise spend a revision" below, whose steps carry the same facts: the increment (fact: live `Revisions used` exceeds the row's frozen `<M>`), the round-`<N>` snapshot write (its own once-per-round rule, unchanged), the writer/decomposer dispatch (fact: the on-disk artifacts differ from round `<N>`'s snapshot — a revision that changed nothing leaves no fact and is re-dispatched, still spending the one revision the row froze), and the Phase-4 re-review (fact: a `## Round <N+1>` heading already in the review log); mark `applied` once that last fact is present, then evaluate rules 2–4 against the round that re-review recorded — the extension buys an ordinary round, not an exemption from these rules. **accept** fingerprints every residual blocking finding under `## Accepted warnings` (rule 3's mechanism, reused unchanged — that section is a fingerprint set, so one already recorded is never written twice) and sets `**Status:** Converged` (fact: the charter's status has left `In review`), then marks the row `applied`, ends `CHARTER — Converged with warnings`, and goes to Phase 6; should a resume land between those last two writes, the State model sends the converged charter to Phase 6 and the row simply stays `pending` — inert, since this rule is only ever consulted while the charter is still `In review`. **stop** ends `CHARTER — Blocked` (max rounds), residual findings listed, and marks the row `applied` — it writes no artifact state, so it is idempotent by construction and safe to re-emit on every resume. The gate itself is asked at most once per cap value — a later hit at a newly raised `<cap>` is a new pair, asked fresh. Never claim convergence because retries ran out, regardless of which choice was made.
