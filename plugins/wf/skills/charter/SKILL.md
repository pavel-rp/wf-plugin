...
4. **Blocking findings** (round 1: CRITICAL/HIGH routed to `charter-writer` or `decomposer`; round ≥2: `blocking: yes` — CRITICAL anywhere, HIGH on a section the reviewer's snapshot diff marked changed, or the OUT-4 size-budget-overrun checklist row (SUB-3), which blocks in every round irrespective of the changed-text rule):
   ...
   - **Round cap:** Check `03_review-log.md`'s `## Cap-gate decisions` first — each row has the mechanical, fully-labeled form `- Round <N> | revision: <M> of <cap> | choice: extend|accept|stop | status: pending|applied` (mirroring `## Growth authorizations`'s labeled-field shape), `<M> of <cap>` frozen at the values read when the row was created. A `status: pending` row, or any `choice: stop` row regardless of status, needs resolving now — skip straight to its branch below. Otherwise, if `Revisions used` has reached the header's `<cap>` (starts at 3; only ever raised by an extend) with blocking findings left, this is a fresh cap hit: **headless** stops `CHARTER — Blocked` (max rounds), residual findings listed, no gate, exactly as before; **interactive** asks once via `AskUserQuestion` (*extend by one revision* / *accept the residual as warnings* / *stop*) and appends the choice as a new `status: pending` row (create the section on first use) *before* acting, then proceeds to its branch below. Every branch acts on the blocking findings already recorded under this cap-hit round's own `## Round <N>` heading — never a fresh review to re-derive them. **One re-entrancy rule governs every branch:** each step below names the durable marker its own completion leaves behind — the same kind of marker this rule already uses elsewhere (`consumed:`, the round snapshot, the publish ledger), never a new state of its own; perform a step only while its marker is absent, skip it once present, and mark `applied` when the last step's marker is present, so one recorded choice yields one outcome whether the branch runs straight through or a resume lands mid-sequence. `applied` therefore means every marker is on disk and no step remains (a `stop` row is still re-entered, but only to re-emit a terminal block that mutates nothing); `pending` means re-enter here and let the markers decide. **extend** — raise `<cap>` by 1 in place (marker: the header's live `<cap>` exceeds the row's frozen `<cap>`; the grepped `M of cap` header shape unchanged — C020's `11 of 11 (8 user-authorized extensions)` is the prior art), then fall through to "Otherwise spend a revision" below, whose steps carry their own markers: the increment (marker: live `Revisions used` exceeds the row's frozen `<M>`), the round-`<N>` snapshot write (its own once-per-round rule, unchanged), then that bullet's dispatch sequence — which is **not** one marker but one per artifact it can write, since that bullet may require two conditionally-dependent dispatches: the writer (marker: `01_charter.md` differs from `01_charter.round-<N>.md`), the decomposer, dispatched on the straight-through path exactly when that bullet's own condition calls for it (marker: `02_subtasks.md` differs from `02_subtasks.round-<N>.md`), and that bullet's id-diff, whose marker is every `## Growth authorizations` entry recorded for this round reading `consumed: yes` — an entry still reading `consumed: no` is indistinguishable from one never diffed, so the diff simply re-runs, which is safe because it is a pure comparison against the round-`<N>` snapshot and reaching the same verdict twice changes nothing. Because `Scope changed: yes` is a dispatch-time report that reaches no artifact, a resume finding the writer's marker present and the decomposer's absent cannot tell *not required* from *not yet run*, so it **re-dispatches the decomposer**: re-running a decomposition that was already correct costs one dispatch, whereas skipping a required one carries forward exactly the invalidated decomposition that bullet forbids. Last, the Phase-4 re-review (marker: a `## Round <N+1>` section heading this log's own append step wrote — matched anchored at the start of a line, never as a substring, so quoted reviewer evidence can never forge it); mark `applied` once that marker is present, then evaluate rules 2–4 against the round that re-review recorded — the extension buys an ordinary round, not an exemption from these rules. **accept** fingerprints every residual blocking finding under `## Accepted warnings` (rule 3's mechanism, reused unchanged — that section is a fingerprint set, so one already recorded is never written twice) and sets `**Status:** Converged` (marker: the charter's status has left `In review`), then marks the row `applied`, ends `CHARTER — Converged with warnings`, and goes to Phase 6; should a resume land between those last two writes, the State model sends the converged charter to Phase 6 and the row simply stays `pending` — inert, since this rule is only ever consulted while the charter is still `In review`. **stop** ends `CHARTER — Blocked` (max rounds), residual findings listed, and marks the row `applied` — it writes no artifact state, so it is idempotent by construction and safe to re-emit on every resume. The gate itself is asked at most once per cap value — a later hit at a newly raised `<cap>` is a new pair, asked fresh. Never claim convergence because retries ran out, regardless of which choice was made.
   - Otherwise spend a revision (increment `Revisions used`): first write the round-just-reviewed snapshot — the current on-disk `01_charter.md`/`02_subtasks.md` to `{charter-folder}/snapshots/01_charter.round-<N>.md` / `02_subtasks.round-<N>.md` (`N` = the round just reviewed) — this is what round `N+1`'s reviewer diffs against, and **every** reference to this write, here and in rule 1, performs it **at most once per round**: skip it when round `N`'s snapshot files already exist, so no later dispatch in the same round overwrites the true pre-round baseline. Then, if any blocking finding routes to `charter-writer`, dispatch the writer (Phase 2, revision) with those findings; if the writer reports `Scope changed: yes`, always re-dispatch the decomposer (Phase 3, revision) afterwards — a changed charter invalidates the decomposition. If findings route only to `decomposer`, dispatch it alone. Diff active `OUT`/`SUB` ids against the snapshot just written, disregarding any id already marked `consumed: yes` earlier in this round: no new id → proceed; exactly one new id whose outcome/sub-task text plausibly addresses the stated `<gap>` of a `granted, consumed: no` `## Growth authorizations` entry recorded for this round → mark that entry `consumed: yes` (one entry authorizes exactly one id) and proceed; any other new id → unauthorized growth — raise a user-routed check to retroactively authorize (record a fresh `consumed: yes` entry) or retire the extra id(s) back (headless: `CHARTER — Needs input`, never silently legitimized). Then re-review the **full artifact set** (Phase 4) — consistency is a cross-artifact property; never re-review only the patched part.
...
**Direct provider resolution** (how the tracker ops are reached): every operation below (`get`, `create_umbrella`, `create_child`, `update`, `list_children`, `post_comment`) reaches the `tracker` surface via the direct provider resolution procedure `invocation-runtime.ops.md` §"Direct provider resolution" defines, reusing the `workspaceRoot` captured in Prerequisites — core names only the abstract operations, never a tracker. If the `wf-resolver` service is unavailable, stop and report that the resolver runtime is not loaded.
