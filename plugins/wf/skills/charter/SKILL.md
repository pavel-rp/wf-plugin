...
| Both present, `**Status:** In review`, and `03_review-log.md`'s last `## Cap-gate decisions` row is `status: pending`, or `choice: stop` (any status) | Phase 5 rule 4's recorded-choice branch directly (resumes or re-emits that row's outcome, acting on the blocking findings already recorded under that cap-hit round's own `## Round <N>` heading) — never re-dispatch Phase 4 |
...
4. **Blocking findings** (round 1: CRITICAL/HIGH routed to `charter-writer` or `decomposer`; round ≥2: `blocking: yes` — CRITICAL anywhere, HIGH on a section the reviewer's snapshot diff marked changed, or the OUT-4 size-budget-overrun checklist row (SUB-3), which blocks in every round irrespective of the changed-text rule):
   ...
   - **Round cap:** Check `03_review-log.md`'s `## Cap-gate decisions` first — each row has the mechanical, fully-labeled form `- Round <N> | revision: <M> of <cap> | choice: extend|accept|stop | status: pending|applied` (mirroring `## Growth authorizations`'s labeled-field shape), `<M> of <cap>` frozen at the values read when the row was created. A `status: pending` row, or any `choice: stop` row regardless of status, needs resolving now — skip straight to its branch below (a stop row's outcome is a stable terminal, safe to re-emit any number of times; an `applied` extend/accept row needs nothing further and is never "the last entry" once its own branch has actually finished). Otherwise, if `Revisions used` has reached the header's `<cap>` (starts at 3; only ever raised by an extend) with blocking findings left, this is a fresh cap hit: **headless** stops `CHARTER — Blocked` (max rounds), residual findings listed, no gate, exactly as before; **interactive** asks once via `AskUserQuestion` (*extend by one revision* / *accept the residual as warnings* / *stop*) and appends the choice as a new `status: pending` row (create the section on first use) *before* acting, then proceeds to its branch below. Every branch acts on the blocking findings already recorded under this cap-hit round's own `## Round <N>` heading — never a fresh review — and marks the row `status: applied` only once its entire outcome below is reached, never earlier: **extend** — if the header's *current* `<cap>` already exceeds the row's frozen `<cap>`, the raise already happened (a resumed pending row), so skip straight to "Otherwise spend a revision" below; otherwise raise `<cap>` by 1 in place now (the grepped `M of cap` header shape unchanged — C020's `11 of 11 (8 user-authorized extensions)` is the prior art) and fall through to it — mark `applied` only after that bullet's full sequence, through its re-review dispatch, completes; **accept** fingerprints every residual blocking finding under `## Accepted warnings` (rule 3's mechanism, reused unchanged), sets `**Status:** Converged`, marks the row `applied`, ends `CHARTER — Converged with warnings`, and goes to Phase 6; **stop** ends `CHARTER — Blocked` (max rounds), residual findings listed, and marks the row `applied` (idempotent to repeat on every resume). The gate itself is asked at most once per cap value — a later hit at a newly raised `<cap>` is a new pair, asked fresh. Never claim convergence because retries ran out, regardless of which choice was made.
