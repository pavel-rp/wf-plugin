...
interview → writer → decomposer → reviewer ─┬─ CLEAN or accepted warnings → publish → done
                ...
                └────────────┴──────────────┤─ user-routed questions → ask, fold in, revise
                                            └─ no progress → stop honestly; rounds exhausted → the cap gate (extend / accept / stop)
```
...
| Both present, `**Status:** In review`, and `03_review-log.md`'s last entry is a `## Cap-gate decisions` row with `status: pending` | Phase 5 rule 4's recorded-choice branch directly (resumes that row's follow-through, acting on the blocking findings already recorded under that cap-hit round's own `## Round <N>` heading) — never re-dispatch Phase 4 |
...
4. **Blocking findings** (round 1: CRITICAL/HIGH routed to `charter-writer` or `decomposer`; round ≥2: `blocking: yes` — CRITICAL anywhere, HIGH on a section the reviewer's snapshot diff marked changed, or the OUT-4 size-budget-overrun checklist row (SUB-3), which blocks in every round irrespective of the changed-text rule):
   ...
   - **Round cap:** if `Revisions used` has reached the header's `<cap>` (starts at 3; only ever raised by an extend below) with blocking findings left, this is a cap hit. Check `03_review-log.md`'s `## Cap-gate decisions` first — each row has the mechanical form `- Round <N> | <M> of <cap> | choice: extend|accept|stop | status: pending|applied` (mirroring `## Growth authorizations`'s row shape): a row for this exact `<M> of <cap>` pair with `status: pending` means resume its follow-through directly below, no re-ask (this is what a `/clear`-and-resume honors — State model); one already `status: applied` means this exact decision already ran — do nothing further, never re-trigger it. No matching row: **headless** — stop `CHARTER — Blocked` (max rounds), residual findings listed, exactly as before; **interactive** — ask once via `AskUserQuestion` (*extend by one revision* / *accept the residual as warnings* / *stop*) and append the choice as a `status: pending` row to `## Cap-gate decisions` (create on first use) *before* acting. Then, whichever path recorded or resumed the choice, acting on the blocking findings already recorded under this cap-hit round's own `## Round <N>` heading (never a fresh review): **extend** raises `<cap>` by 1 in place (the grepped `M of cap` shape unchanged — C020's `11 of 11 (8 user-authorized extensions)` is the prior art) and falls through to "Otherwise spend a revision" below; **accept** fingerprints every residual blocking finding under `## Accepted warnings` (rule 3's mechanism, reused unchanged), sets `**Status:** Converged`, ends `CHARTER — Converged with warnings`, and goes to Phase 6; **stop** ends `CHARTER — Blocked` (max rounds), residual findings listed. Mark the row `status: applied` the moment its branch's action is taken — before doing anything else — so a second resume finds nothing left to do. The gate fires at most once per cap value — a later cap hit at the newly raised `<cap>` is a new pair, asked fresh. Never claim convergence because retries ran out, regardless of which choice was made.
