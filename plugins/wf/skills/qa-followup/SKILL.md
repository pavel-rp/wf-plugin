...
1. **Unblock** — change the *test harness* to clear the block: `/wf:qa-host augment` an existing host to expose a control or observation a scenario needs (the common case — a host that pins `showLabel=true` or hardcodes `selectedReportPeriodIds` is an augment, not an escalation), `/wf:qa-host new` to scaffold a missing one, `/wf:qa-host api-probe` to (re-)wire or resolve a backend endpoint for a `Type: API` scenario blocked on `Backend host required:` (or one BLOCKED because the API hadn't rebuilt), or just re-run after a transient session drop. Then re-execute those scenarios via `/wf:qa-auto --only`. New failures fold into the defect set.
...
- Invoke `/wf:qa-host` (`route` / `new` / `augment` for Angular hosts; `api-probe` / `api-revert` for backend endpoints) to scaffold or resolve a test surface, and `/wf:qa-auto --only` to re-run unblocked scenarios. These own their own write permissions (including the ephemeral controller wiring, which `wf:qa-host` reverts).
...
- **Host capability gap** — a host exists but the scenario can't run because it **pins an `@Input`** the scenario must vary, **lacks a control** to reach a required state, or **doesn't surface an `@Output`** the scenario must observe (e.g. "host hardcodes `showLabel=true`", "no control to set `selectedReportPeriodIds`", "`userAccessLevelChanged` never observed"). → **augment the host** (Phase 5). **This is the common case, and augmenting the harness is this skill's primary job — escalate a host gap only when augmenting is genuinely impossible (see ESCALATE).** A gap is mechanically fixable whenever the needed control/observation maps to a public `@Input`/`@Output` on the target.
- **`Host required: <component>` not scaffolded / host typecheck failed** → scaffold the test-host (Phase 5).
- **`Backend host required:` not wired, or `setup: backend host wired but API not rebuilt`** (a `Type: API` scenario) → re-`/wf:qa-host api-probe` the service, and if the block was "API not rebuilt", note that the API must be restarted / run under `dotnet watch` before the re-run. This is a harness block, not a product defect — the endpoint behavior was never reached.
...
The scenario is `FAIL` (observed ≠ expected at a step, or — for a `Type: API` scenario — the endpoint returned the wrong status/shape against an existing or correctly-wired endpoint). The fix is a source change, planned in Phase 6: a browser FAIL is root-caused in the component/service; an **API FAIL is root-caused in the controller/service/repository** the scenario exercised. A `Type: API` scenario that failed *only* because the endpoint wasn't wired or the API hadn't rebuilt is UNBLOCK, not DEFECT — the behavior was never reached. Newly-FAILed scenarios surfaced by the Phase 5 re-run join this bucket.
...
- **Host gaps that augmenting can't reach:** the required state can't be produced through any public `@Input`/`@Output` (the component would need an internal change to be testable), or the host still fails to typecheck after an augment attempt. A host gap that *does* map to the target's public surface is UNBLOCK, never ESCALATE — do not escalate a `showLabel`/`selectedReportPeriodIds`-style pinned-input gap; augment it.
...
**First, separate harness gaps from product defects.** If a defect's failing observation is a missing or pinned *test-host* affordance — a control or readout the host scaffold should provide, not a behavior of the component under test (e.g. observed "no toggle for `showLabel`", "cannot set `selectedReportPeriodIds`", "`userAccessLevelChanged` count never shown") — it is a harness gap, not a product bug. Reclassify it to UNBLOCK, `/wf:qa-host augment` the host (Phase 5), re-run, and do **not** plan a product fix. Only the residue — divergences in the component's *own* behavior — proceeds through the steps below.
...
|---|---|---|---|
| TC-003 | Host required: FooComponent | scaffolded via /wf:qa-host new | PASS |
| TC-005 | host pins showLabel=true | augmented: /wf:qa-host augment --control showLabel | PASS |
| TC-006 | userAccessLevelChanged not observed | augmented: --observe userAccessLevelChanged | FAIL@step3 → defect FIX-002 |
...
- **Host exists but is inadequate for the scenario** (pinned input, missing control, unwired output) — this is UNBLOCK, not ESCALATE: `/wf:qa-host augment` the host with the right `--control`/`--observe`/`--show` flags and re-run. Escalate only if the needed state can't be reached through the component's public `@Input`/`@Output` surface.
- **`/wf:qa-host` reports the component already has a host but the scenario still blocks on it** — the block isn't "no host", so don't re-scaffold (`new` won't touch an existing host anyway). If it's a capability gap (pinned input / missing control / unwired output), `augment` the host and re-run. Only if the block is structural (routing/DI broken, or the needed state isn't reachable through the public surface) reclassify to ESCALATE.
...
- **`Type: API` scenario BLOCKED on `backend host wired but API not rebuilt`** — UNBLOCK the wiring via `api-probe`, but the re-run can't reach the endpoint until the API recompiles, which this skill can't trigger. Re-run once; if still BLOCKED for the same reason, ESCALATE with remedy: "restart the API (or run `dotnet watch`) so the `__qa` route goes live, then `/wf:qa-auto <id> --only <TC>`." Don't reclassify it as a defect — the behavior was never exercised.
