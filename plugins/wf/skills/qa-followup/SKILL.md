...
1. **Unblock** — change the *test harness* to clear the block: `/wf-caps:qa-host augment` an existing host to expose a control or observation a scenario needs (the common case — a host that pins `showLabel=true` or hardcodes `selectedReportPeriodIds` is an augment, not an escalation), `/wf-caps:qa-host new` to scaffold a missing one, `/wf-caps:qa-host api-probe` to (re-)wire or resolve a backend endpoint for a `Type: API` scenario blocked on `Backend host required:` (or one BLOCKED because the API hadn't rebuilt), or just re-run after a transient session drop. Then re-execute those scenarios via `/wf:qa-auto --only`. New failures fold into the defect set.
...
- Invoke `/wf-caps:qa-host` (`route` / `new` / `augment` for frontend test hosts; `api-probe` / `api-revert` for backend endpoints) to scaffold or resolve a test surface, and `/wf:qa-auto --only` to re-run unblocked scenarios. These own their own write permissions (including the ephemeral controller wiring, which `wf-caps:qa-host` reverts).
...
**First, separate harness gaps from product defects.** If a defect's failing observation is a missing or pinned *test-host* affordance — a control or readout the host scaffold should provide, not a behavior of the component under test (e.g. observed "no toggle for `showLabel`", "cannot set `selectedReportPeriodIds`", "`userAccessLevelChanged` count never shown") — it is a harness gap, not a product bug. Reclassify it to UNBLOCK, `/wf-caps:qa-host augment` the host (Phase 5), re-run, and do **not** plan a product fix. Only the residue — divergences in the component's *own* behavior — proceeds through the steps below.
...
| TC-005 | host pins showLabel=true | augmented: /wf-caps:qa-host augment --control showLabel | PASS |
