...
1. **Unblock** — change the *test harness* to clear the block via the registered `qa-execution` host provider (resolved by the `wf-resolver` `resolve_provider("qa-execution:host")` query, which returns the owner of the `host` surface): its `augment` operation exposes a control or observation a scenario needs (the common case — a host that pins an input value or hardcodes a value the scenario must vary is an augment, not an escalation), its `new` operation scaffolds a missing host, its `api-probe` operation (re-)wires or resolves a backend endpoint for a `Type: API` scenario blocked on `Backend host required:` (or one BLOCKED because the API hadn't rebuilt), or just re-run after a transient session drop. Then re-execute those scenarios via `/wf:qa-auto --only`. New failures fold into the defect set.
...
1. **Apply harness fixes** by dispatching to the registered `qa-execution` host provider (resolved via the `wf-resolver` `resolve_provider("qa-execution:host")` query, which returns the run-scoped record `{ owner, fragmentPath, state, … }` for the `host` surface — the resolver has already walked the `## Capabilities` registry and every `manifest.md`; core never names or reads the provider's own skill file, it dispatches to whatever `owner` owns the `host` surface). On `state: unconfigured`/`unrecoverable` (no host provider registered), the graceful no-host path applies — leave the scenario blocked with its scope reason and escalate, exactly as when augmenting can't reach the target's public surface. If the `wf-resolver` service is unavailable, stop and report that the resolver runtime is not loaded (WF-272 diagnostics/recovery).
