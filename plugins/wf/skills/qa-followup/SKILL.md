...
1. **Unblock** — change the *test harness* to clear the block via the registered `qa-execution` host provider (the `provider` fragment owning `surface: host` in the `## Capabilities` registry): its `augment` operation exposes a control or observation a scenario needs (the common case — a host that pins an input value or hardcodes a value the scenario must vary is an augment, not an escalation), its `new` operation scaffolds a missing host, its `api-probe` operation (re-)wires or resolves a backend endpoint for a `Type: API` scenario blocked on `Backend host required:` (or one BLOCKED because the API hadn't rebuilt), or just re-run after a transient session drop. Then re-execute those scenarios via `/wf:qa-auto --only`. New failures fold into the defect set.
...
1. **Apply harness fixes** by dispatching to the registered `qa-execution` host provider (resolved from the `## Capabilities` registry; core never names or reads the provider's own skill file — it invokes whatever owns `surface: host`).
