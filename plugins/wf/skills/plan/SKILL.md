...
**Step 2b — Write `02_plan.md`** using the Plan Template at `plan-template.md`, obtained via the resolver's `resolve_content` (`class: references-template`, `skill: plan`, `ref: plan-template.md`), never a raw `Read` of the plugin-cache path — see "Plan Template" below. **Overwrite if it exists** (version-control history preserves prior versions).
