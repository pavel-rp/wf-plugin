...
Derive `<branch-name>` from Step 2 (unchanged, tracker-side logic — out of scope for this rewrite), then invoke `branch-create(<branch-name>)` against the resolved `delivery` record (obtain its body via `resolve_content` and follow it in-context).
