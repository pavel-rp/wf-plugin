...
2. Apply the shared two-arm-plus-collision-guard predicate (obtain the doc via `resolve_content({ workspaceRoot, ... })`, `class: shared`, `ref: pipeline-conventions.md` — never a raw `Read` of the plugin-cache path) §"Branch gate (bare-core aware)" step 2 to `<branch>`, compared case-insensitively: a `/{task-id}-` match (where `{task-id}` differs from `{numeric-id}`) is accepted outright, and a `/{numeric-id}-`-only match is accepted only when the guard's scan of `{task-root}` and `{task-root}/_archive/` finds no folder whose own first-3+-digit run equals `{numeric-id}` but whose full name differs from `{task-id}`. Do not restate the predicate beyond this summary; the shared section is authoritative so the guard cannot drift out of sync. Not accepted — no match, or a numeric-only match the guard rejected → `PR — Error`, reason "Not on the task branch. Run /wf:pr without --no-commit, or /wf:branch first." The lower-casing is for comparison only and never changes what is written or emitted — the PR title, the `Task:` line, and every tracker operation keep `{task-id}` verbatim.
