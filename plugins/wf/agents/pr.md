...
3. `<base>`: the `base` input, else the repository's default base resolved via the `default-base-query` read operation against the `delivery` record already resolved above (obtain its body via `resolve_content` and follow it in-context; on `state: unconfigured`/`unrecoverable`, a plain default base). **Never name a trunk here** — core does not assume the repository's default-branch name.
