...
3. **Zero `tracker` owner** (self-resolve matched no row, or the forwarded record marks the surface unconfigured). Silent local-only fallback: no `get`/`attach_link` call is attempted, and Step 3's Work-item link section and "Resolves…" sentence are both omitted entirely — no capability term appears anywhere in the output.
