...
- **`MODE: ci`** — followed by **either** a run reference (a run/check identifier plus the owning repository and PR number) to fetch yourself, **or** a pre-captured raw failing-log blob, **or** a path to a captured-log file.
  - When given a run reference **and** the active delivery provider exposes a CI-log read operation, resolve that operation and fetch the failing log **through the provider, in your own context**, so the log never touches the caller.
  - When **no delivery provider is registered** (or none exposes such a read), the caller hands you the bulk directly — read the given file path or inline blob with `Read`/`Grep`. Degrade silently to this local path; never surface a provider-absence error.
