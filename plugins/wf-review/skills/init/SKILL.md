---
name: init
description: Onboards the wf-review pack by entering the canonical /wf:init lifecycle with wf-review seeded into the selection round, so a project gets the same discovery, question, delta, confirmation and apply it would get from /wf:init itself. Preserves every registration the project already has and adds wf-review to them; it decides nothing about the pack's state, asks nothing of its own, and performs no registry write. The pack ships the pr-review capability. Use after /wf:init to register pr-review so a future contribution fragment resolves; re-run any time — a re-run over a settled project reports no drift and mutates nothing. /wf:init is the canonical command and does the same thing for every pack at once.
allowed-tools: [Skill, Bash]
---

# /wf-review:init — Onboard the wf-review pack (a compatibility alias onto the shared lifecycle)

This skill is a **compatibility alias**: it contributes exactly one thing to the
canonical setup lifecycle — "add `wf-review` to the desired set" — and then gets
...
> **What this skill does not decide.** Whether `wf-review` is installed, enabled,
...
**This pack asks nothing.** The `pr-review` capability declares no interview
...
**Why register a capability that ships no fragment yet.** `pr-review`'s two
skills already reach users by native plugin composition alone — no registry row
is needed for *them* to run. A future contribution presupposes a registration
path, so registering now makes `pr-review` resolvable ahead of that fragment
landing. Registering it is exactly what the canonical apply does; this skill only
names the pack.
...
1. **Route the edge, then enter the lifecycle.** Run `pwd -P` once and hold the
   ...
   `unitIds: ["pr-review:init"]`, `shapeEvidence: { workSurface:
   ...
   `/wf:init --seed wf-review` through the **Skill** tool.

2. **Relay what comes back, verbatim.** The `INIT` block is this skill's Final
   Output. Add the review note below it and nothing else — no re-derived status,
...
- **`/wf:init` has not run yet:** not a precondition this skill checks. `/wf:init`
  *is* what is being invoked, and it scaffolds the bare core itself before any
  pack transaction.
- **`wf-review` is disabled:** the seed is reported *not applied*; the pack stays
...
- **`wf-review` is already set up and the project is settled:** the canonical
...
- **`pr-review` carries no fragment rows yet:** expected — the capability registers
  ahead of its first contribution. Registry validation passes regardless, and no
  review term surfaces in any core skill run until that fragment lands.
...
- **The project already has other packs set up:** they are all preserved. Entering
  through this command **adds** `wf-review` to them and deregisters nothing —
...
Review note:
- pr-review registers ahead of its first contribution fragment, so the row the canonical apply writes is what lets that fragment resolve when it lands.
```
...
Seed: wf-review — not applied (alias could not enter the canonical lifecycle)
Reason: <the resolver diagnostic, or the invocation error, verbatim>

Next: resolve the reason above, then re-run /wf-review:init.
```

**Both blocks are breaking replacements for the previous
`WF-REVIEW-INIT — <status>` block** (MINOR, pre-1.0): one shared route has one
