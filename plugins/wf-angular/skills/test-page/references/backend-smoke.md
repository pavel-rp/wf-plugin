# wf-angular:test-page — `backend-smoke` subcommand

Smoke-test a newly added .NET controller endpoint by adding a thin Angular service method and wiring it into the wf-angular:test-page harness.
...
5. **Write the page-test file** in `_page-tests/<suite-name>.page-test.ts`. Tests are async and call the real API via the service method. Assert:
   ...
   - Edge cases from the spec (e.g., "empty array when no records exist" — use an id unlikely to have data, such as `0` or `-1`).
