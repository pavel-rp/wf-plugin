---
name: init
description: Onboards the wf-audit pack by entering the canonical /wf:init lifecycle with wf-audit seeded into the selection round, so a project gets the same discovery, question, delta, confirmation and apply it would get from /wf:init itself. Preserves every registration the project already has and adds wf-audit to them; it decides nothing about the pack's state, asks nothing of its own, and performs no registry write. The pack ships the audit and sr capabilities together. Use after /wf:init to activate the five verify-phase adversarial lenses plus the pre-commit self-review lens; re-run any time — a re-run over a settled project reports no drift and mutates nothing. /wf:init is the canonical command and does the same thing for every pack at once.
allowed-tools: [Skill, Bash]
---

# /wf-audit:init — Onboard the wf-audit pack (a compatibility alias onto the shared lifecycle)

This skill is a **compatibility alias**: it contributes exactly one thing to the
canonical setup lifecycle — "add `wf-audit` to the desired set" — and then gets
out of the way. Everything else, from admitting the workspace root to the single
`apply_install` that registers the capabilities, belongs to `/wf:init` and happens
there.
...
> **What this skill does not decide.** Whether `wf-audit` is installed, enabled,
...
**This pack asks nothing.** Neither the `audit` nor the `sr` capability declares an
interview question, so the canonical question round asks nothing on their behalf
...
run, and it belongs to `/wf:init`. Profile data the capabilities ship keeps its
working defaults and is not a question.

**One seed covers both capabilities.** The pack ships `audit` and `sr` as one unit
— `sr` reaches `audit`'s owned correctness rubric by a co-located intra-plugin
path — and the seed names the **pack**, not a capability. There is no
capability-subset argument and no per-capability call: the canonical route
discovers what the pack declares and registers it in the one canonical apply.
...
1. **Route the edge, then enter the lifecycle.** Run `pwd -P` once and hold the
   ...
   `workspaceRoot: "<workspace-root>"`, `role: "init"`, `unitIds: ["audit:init"]`,
   ...
   no selector, and invoke `/wf:init --seed wf-audit` through the **Skill** tool.

2. **Relay what comes back, verbatim.** The `INIT` block is this skill's Final
   Output. Add the lens note below it and nothing else — no re-derived status,
...
- **`/wf:init` has not run yet:** not a precondition this skill checks. `/wf:init`
  *is* what is being invoked, and it scaffolds the bare core itself before any
  pack transaction.
- **`wf-audit` is disabled:** the seed is reported *not applied*; the pack stays
...
- **`wf-audit` is already set up and the project is settled:** the canonical
...
- **Only one of the pack's two capabilities is already registered:** not a state
  this skill reads or reports. The canonical delta covers whatever the pack
  declares against whatever the project has, and the canonical apply reconciles it.
...
- **The project already has other packs set up:** they are all preserved. Entering
  through this command **adds** `wf-audit` to them and deregisters nothing —
...
Lens note:
- audit contributes the five verify-phase lenses and sr the pre-commit self-review lens; the pack ships them as one unit, so one seed covers both.
```
...
Seed: wf-audit — not applied (alias could not enter the canonical lifecycle)
Reason: <the resolver diagnostic, or the invocation error, verbatim>

Next: resolve the reason above, then re-run /wf-audit:init.
```

**Both blocks are breaking replacements for the previous `WF-AUDIT-INIT — <status>`
