...
Its core pair — `inspect_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-audit" })` (read-only — resolves the plugin via
`claude plugin list --json`, validates it, and returns a fingerprint) then
`register_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-audit", expectedFingerprint: <fingerprint> })` (the sole mutation — writes the `## Plugin
...
Call `inspect_pack` with `workspaceRoot: "<workspace-root>"` and `pluginId: "wf-audit"` — this pack's exact stable plugin id
...
1. Call `resolve_registry({ workspaceRoot: "<workspace-root>" })` and note which of `audit`/`sr` already appear as **active**
...
2. Call `register_pack` with `workspaceRoot: "<workspace-root>"`, `pluginId: "wf-audit"`, and `expectedFingerprint` = the
...
- **audit** (if newly registered in Phase 2): resolve its manifest at the `manifestPath`
  ...
  `resolve_content` (`workspaceRoot: "<workspace-root>"`, `class: profile-template`, `capability: audit`) — never a raw
...
2. `selfCheck: "failed"` → call `resolve_registry({ workspaceRoot: "<workspace-root>" })` again and find the entry (or entries)
