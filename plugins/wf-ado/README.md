...
`list_children`, `post_comment`, `set_status`, `attach_link` — to concrete Azure
DevOps mechanics, so a `wf` core skill that needs to read or write a work item
has a provider to dispatch to.
...
| `capabilities/ado/fragments/tracker.md` | the inline reference doc binding all nine tracker operations to Azure DevOps mechanics, with a completeness coverage table |
