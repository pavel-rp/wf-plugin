...
**Version:** 1.0.0 (WF-123 — initial tracker-provider capability, binding SUB-2/WF-121's
`tracker` contract to concrete Azure DevOps mechanics)
...
`plugins/wf-ado/capabilities/ado/fragments/tracker.md`). `scope` is required for
...
- **tracker** (`spec | provider | inline: fragments/tracker.md | tracker`) — the Azure
...
id scheme, per `capability-registry.contract.md` §"The tracker provider surface".
