...
The `ado` capability's fill for the `implement.milestone` slot — the **only `append`-policy point
...
**Role framing.** The `Impl:` record's **comment thread is the running implementation log.** Each
