...
**Version:** 1.0.0 (WF-413 — the `plan.publish` half of the C021 mid-conveyor mirror, authored to parity with the `linear` fill; review-verified, never run live)
...
The `ado` capability's fill for the `plan.publish` slot (`replace` policy). `/wf:plan`
...
and never invalidates the plan: `02_plan.md` is already written and remains the source of truth.

**Reconciliation with the other artifact fills — read this before changing anything.** The
`spec.publish` fill creates a `Spec:` child; this fill creates a **`Plan:`** child, and
`tasks.publish` creates a **`Tasks:`** child. Each writes a **distinct** child work item and
records its id under a **distinct** metadata key in its **own** local artifact, so no two fills
ever touch the same field of the same item and nothing is double-published. This fill must
**never** patch the task's own description — the `spec` Phase-0 backfill remains the only write
to it.
...
## Step 3 — Create the plan child work item carrying the full artifact
...
Invoke `update(<plan-item-id>, tags: ["wf-artifact"])` to mark the child as a published
...
Invoke `set_status(<plan-item-id>, <the project's completed state>)` — `Closed` in the Agile
