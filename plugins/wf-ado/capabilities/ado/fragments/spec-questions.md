...
**Version:** 1.0.0 (WF-413 — the `spec.questions` half of the C021 tracker mirror, authored to parity with the `linear` fill; review-verified, never run live)
...
The `ado` capability's fill for the `spec.questions` slot (`replace` policy). `/wf:spec`
...
work item, changes no state, patches no field, and never blocks the phase: whatever happens
...
3. Neither holds — the task has no tracker record yet (a local `T<NNN>` id). **Skip this fill
   ...
   this fill's: a question comment must not be the thing that mints a work item.
...
`spec.publish`, and the authored-not-tested status of this whole slot set:
