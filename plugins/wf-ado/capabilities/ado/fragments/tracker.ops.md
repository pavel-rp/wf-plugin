...
1. **`ado-organization` and `ado-project` — from the profile.** Call `resolve_profile({ workspaceRoot, capability: "ado" })` — the resolver's typed, override-merged **values** view. Both are **declared questions the install lifecycle asks and persists** (`profile.template.json` `ask[]`), so the capability profile is their authoritative home: reading an answered question from anywhere else is what makes it get asked again.
2. **Read-through fallback, for existing projects only.** When the profile yields no value for one of them, fall back to the `## Azure DevOps` config section: call `resolve_config({ workspaceRoot })` (R1) to obtain `registryPath` — never assume the `_local/config.md` literal; a project's `wf.config.js` may relocate it — and read the section there. On a resolver failure (snapshot-missing/malformed/schema-incompatible/etc.), call `resolve_gate` with `{ surface: "local-read", workspaceRoot }` and follow its `reaction` — a local read continues best-effort per `capability-registry.ops.md` §"Resolver-failure semantics" — falling back to the conventional default `_local/config.md` location rather than blocking or prompting. **Profile first, config section second**; a value present in both is decided by the profile, and nothing writes these two back to the config section.
3. **`Work Item ID Prefix` stays in the config section.** It is **not** an asked answer — it is template data carrying the default `ADO` — so it keeps its existing read, from the `## Azure DevOps` section at the resolved `registryPath`.
4. **Configured** — all three values (`ado-organization`, `ado-project`, `Work Item ID Prefix`) resolve to a real value from their own surface: not the `<...>` bracket placeholder shape `/wf:init`'s template uses for an unset value, and not absent everywhere.
5. **Unconfigured** — any one is still a placeholder or absent from every surface. This is the silent local-only fallback the contract's degradation rules define — **no prompt, no error**.
