...
**Version:** 0.2.0 (WF-344 — hermetic in-memory delivery binding: every delivery op returns a scripted response and records its invocation to the op log; no network/git/gh reach. 0.2.0 adds the argument-keyed `{by, map, default?}` response form for ops whose concurrent callers make call-order sequences answer the wrong argument.)
...
4. **Resolve the scripted response** from `scripts.delivery.<op>`:
   ...
   - **Any other single JSON value** (object/string/number/bool) → that is the response.
   - **A JSON array** (an ordered response sequence, e.g. poll outcomes / evolving thread states) → the element at `<call-index>`; past the end, the **last** element repeats (a settled terminal state). A sequence is indexed by call count, blind to arguments — never script one for an op whose calls interleave across concurrent callers with different arguments; use a keyed map there.
