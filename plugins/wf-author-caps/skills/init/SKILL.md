...
1. **Precondition.** Confirm a git repository (`git rev-parse --git-dir`); if this fails, stop:
   ...
   `/wf:init` creates."
2. **Check prior state (reporting only).** Call `resolve_registry({ workspaceRoot: "<workspace-root>" })`; note whether `author-caps`
...
3. **Inspect the plugin.** Call `inspect_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-author-caps" })`. Read-only; returns
...
4. **Register.** Call
   `register_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-author-caps", expectedFingerprint: <fingerprint from step 3> })`.
...
- **Stale fingerprint** (`register_pack` rejects on a fingerprint mismatch): re-run `inspect_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-author-caps" })`
  to get the current fingerprint, then retry `register_pack({ workspaceRoot: "<workspace-root>", pluginId: "wf-author-caps", expectedFingerprint: <current fingerprint> })`.
