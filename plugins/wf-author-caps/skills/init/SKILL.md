---
name: init
description: Onboards the wf-author-caps authoring toolkit by entering the canonical /wf:init lifecycle with wf-author-caps seeded into the selection round, so a project gets the same discovery, question, delta, confirmation and apply it would get from /wf:init itself. Preserves every registration the project already has and adds wf-author-caps to them; it decides nothing about the pack's state, asks nothing of its own, and performs no registry write. Use after /wf:init to register author-caps so its authoring contributions resolve; re-run any time — a re-run over a settled project reports no drift and mutates nothing. /wf:init is the canonical command and does the same thing for every pack at once.
allowed-tools: [Skill, Bash]
---

# /wf-author-caps:init — Onboard the authoring toolkit (a compatibility alias onto the shared lifecycle)

This skill is a **compatibility alias**: it contributes exactly one thing to the
canonical setup lifecycle — "add `wf-author-caps` to the desired set" — and then
...
> **What this skill does not decide.** Whether `wf-author-caps` is installed,
...
**This pack asks nothing.** The `author-caps` capability declares no interview
...
**Why enter the lifecycle for a toolkit that already loads.** The pack's reference
skills — `/wf-author-caps:authoring-guide` and
`/wf-author-caps:authoring-taxonomy` — reach users by native composition alone,
with no registry row involved. But the capability's phase contributions
presuppose a registration path: without one, those contributions could never fire
once added. Entering the canonical lifecycle makes `author-caps` resolvable ahead
of them landing.
...
1. **Route the edge, then enter the lifecycle.** Run `pwd -P` once and hold the
   ...
   `unitIds: ["author-caps:init"]`, `shapeEvidence: { workSurface:
   ...
   `/wf:init --seed wf-author-caps` through the **Skill** tool.
...
- **`/wf:init` has not run yet:** not a precondition this skill checks. `/wf:init`
  *is* what is being invoked, and it scaffolds the bare core itself before any
  pack transaction.
- **`wf-author-caps` is disabled:** the seed is reported *not applied*; the pack
...
- **`wf-author-caps` is already set up and the project is settled:** the canonical
...
- **The capability declares no fragment row yet:** expected — it registers ahead of
  its first contribution. Registry validation passes on a zero-row table, and no
  authoring term surfaces in any core phase until those contributions land.
...
- **The project already has other packs set up:** they are all preserved. Entering
  through this command **adds** `wf-author-caps` to them and deregisters nothing —
...
Authoring note:
- /wf-author-caps:authoring-guide and /wf-author-caps:authoring-taxonomy load by native composition whether or not the capability is registered; registration is what lets its phase contributions resolve.
```
...
Seed: wf-author-caps — not applied (alias could not enter the canonical lifecycle)
Reason: <the resolver diagnostic, or the invocation error, verbatim>

Next: resolve the reason above, then re-run /wf-author-caps:init.
```

**Both blocks are breaking replacements for the previous
`WF-AUTHOR-CAPS-INIT — <status>` block** (MINOR, pre-1.0): one shared route has
