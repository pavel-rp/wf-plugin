...
- Offer a mechanism-signal predicate kind, match operator, or per-kind slot outside the frozen sets
  the schema names. The vocabulary is closed: a signal that needs a kind, slot, or operator the
  schema does not carry is reported as a finding, not invented.
...
| 5 | **Mechanism signals** — the predicates the analysis evaluates | Zero or more. Each names one of exactly **two** frozen predicate kinds — `record_match` or `dispatch_shape` — and no third kind is offered or accepted. Each carries a short id (unique, `[A-Za-z0-9][A-Za-z0-9_.-]*`), a non-empty one-line description, **and the per-kind slots its kind names** — collected and validated here, at interview time, because the loader refuses a signal that declares them incompletely. `record_match`: at least one of `record` (`type` required, `subtype` optional) and `match[]` (each clause needs `field` and `op`; `field` is a dot path into the record or `""` for the whole record serialized; `op` is one of `equals`, `prefix`, `contains`; `value` is one non-empty literal or a non-empty array of them meaning *any of*) — declaring **neither** `record` nor `match` is rejected, since it would count every record in the stream. `dispatch_shape`: `subagent_type`, required. State plainly, here and in the final report, that a declared signal is **validated at load** — for every phase, including `--dry-run` — and **evaluated by the analyze phase**, which emits its `{status, count, basis}` result row. The emitted manifest carries these slots filled; `experiments/scaffold-selftest/experiment.json` is the reference output. |
...
Arms:       <n> (<label>=<ref>, ...)
Signals:    <n> declared — validated at load, evaluated by the analyze phase
