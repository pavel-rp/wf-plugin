...
**Every observed value is unchanged from the pre-WF-423 recording.** The only textual difference is
the not-measured reason, which now says "absent, null or empty" because absence, `null`, and the
empty string became the same answer — a field present but blank is no more a measurable dimension
than a field that is missing. That is the honest outcome and worth stating plainly: WF-423's spec predicted that fixing
the presence predicate would flip `wf374_audit_lens_boots` and `wf374_gated_off_lens_boots` from a
committed `0` to not-measured, on the theory that their zero was itself an artifact of the defect.
**It is not.** Both measure over a real basis — 29 of 38 candidate records in arm A, 38 of 40 in
arm B — so `gated_off_lens_boots: 0` is a genuine zero, and `audit_lens_boots` was never zero at
all (10 and 15). The signal that legitimately lands on the not-measured path, `wf376_dispatch_model_tier`,
reached it before this change too.

So no committed inventory value depended on the null-versus-undefined distinction, and the
narrow-and-report resolution the spec's Open Question turned on is **moot on this data**. The
behaviour is implemented and covered by the hermetic suite; this run does not exercise it. A defect
class can be real and worth closing without any committed number moving — which is exactly the shape
of a false green: the numbers looked right the whole time.
