...
- Read-only resolution via `current-branch-query` (the `wf-resolver` `resolve_provider({ surface: "delivery", workspaceRoot })` query — see "Branch-based id inference") for branch-based id inference, and via `resolve_profile({ capability: "angular", workspaceRoot })` for stack-profile values (see "Stack profile"). Never call `git` directly for branch inference; never hand-parse the registry, a manifest, or the profile files — every fact the resolver supplies comes from a typed tool call.
