...
**Output** — per-scenario **verdict blocks** in the shared report format ([`../../../wf/skills/qa-gen/references/report-format.md`](../../../wf/skills/qa-gen/references/report-format.md)), appended into `07_qa-report.md` incrementally, plus the `QA-ENGINE — <status>` final block. The engine does **not** own the run-level report header / Summary / traceability rollup — that is the orchestrator's job; the engine supplies the per-scenario results the orchestrator merges. (On direct invocation with empty input, the engine writes a complete report itself.)
