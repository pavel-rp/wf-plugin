...
- **Otherwise the scenario PASSes**, and the report carries the `**Visual:**` PASS-path sub-block per [`references/output-format.md`](references/output-format.md): `**Visual:** PASS`, `**Screenshot:**` (the pass-path capture), `**Geometry findings:**` (the compact table, or `none`), and `**Vision review:**` (the rubric verdict). Advisory Layer-A findings appear in that geometry table tagged `advisory` — present but non-failing.
