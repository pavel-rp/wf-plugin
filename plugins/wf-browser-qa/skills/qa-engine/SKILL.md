...
**Output** — per-scenario **verdict blocks** in the shared report format ([`references/output-format.md`](references/output-format.md)), appended into `07_qa-report.md` incrementally, plus the `QA-ENGINE — <status>` final block. The engine does **not** own the run-level report header / Summary / traceability rollup — that is the orchestrator's job; the engine supplies the per-scenario results the orchestrator merges. (On direct invocation with empty input, the engine writes a complete report itself.)
...
The two-layer procedure — **Layer A** deterministic geometry probes (the probe table + hard-fail/advisory verdict authority), **Layer B** the pass-path screenshot + fixed vision rubric, and the exact verdict-wiring (a hard-fail probe or rubric failure FAILs the scenario; otherwise the `**Visual:**` PASS sub-block) — lives at [`references/visual-verification.md`](references/visual-verification.md). It is read **on-demand**, only when the engine reaches a `**Visual:** yes` scenario, so it stays out of the boot body for every non-visual run. Read that file and follow it now, then fold its verdict into the scenario verdict before recording.

### 5b. Verdict block per scenario

Record one block per scenario in `07_qa-report.md` under the appropriate suite section, in the per-suite-results format from [`references/output-format.md`](references/output-format.md):
