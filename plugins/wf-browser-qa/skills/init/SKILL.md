---
name: init
description: Onboards the wf-browser-qa pack by entering the canonical /wf:init lifecycle with wf-browser-qa seeded into the selection round, so a project gets the same discovery, question, delta, confirmation and apply it would get from /wf:init itself. Preserves every registration the project already has and adds wf-browser-qa to them; it decides nothing about the pack's state, asks nothing of its own, and performs no registry write. The browser-qa capability supplies the qa-execution engine surface. Use after /wf:init to activate the browser-automation QA engine; re-run any time — a re-run over a settled project reports no drift and mutates nothing. /wf:init is the canonical command and does the same thing for every pack at once.
allowed-tools: [Skill, Bash]
---

# /wf-browser-qa:init — Onboard the wf-browser-qa pack (a compatibility alias onto the shared lifecycle)

This skill is a **compatibility alias**: it contributes exactly one thing to the
canonical setup lifecycle — "add `wf-browser-qa` to the desired set" — and then
...
> **What this skill does not decide.** Whether `wf-browser-qa` is installed,
...
**This pack asks nothing.** The `browser-qa` capability declares no interview
...
**The engine's test credentials are not a lifecycle question, and this route does
not move them.** `/wf-browser-qa:qa-engine` obtains its credentials on its **own
first run**, from the downstream `_local/qa-creds.md`, at the moment it needs
them. That behaviour lives entirely in the engine, stays exactly where it is, and
is deliberately **outside** the canonical question round: setup does not collect
it, this alias does not collect it, and it is not declared as a question. Folding
it into the lifecycle round would be a capability change — out of scope here, and
wrong on its own terms, since the credential is needed at drive time and not at
registration time.
...
- Prompt for anything. This pack declares no question, so the correct output is
  ...
  forward. In particular, never collect, pre-collect, or ask about the engine's
  test credentials: that is the engine's own first-run path and not a lifecycle
  question.
...
- Write or edit **any** file, including `_local/config.md`, `_local/qa-creds.md`,
  and any profile override. This skill performs no write at all.
...
1. **Route the edge, then enter the lifecycle.** Run `pwd -P` once and hold the
   ...
   `unitIds: ["browser-qa:init"]`, `shapeEvidence: { workSurface:
   ...
   `/wf:init --seed wf-browser-qa` through the **Skill** tool.

2. **Relay what comes back, verbatim.** The `INIT` block is this skill's Final
   Output. Add the engine note below it and nothing else — no re-derived status,
...
- **`/wf:init` has not run yet:** not a precondition this skill checks. `/wf:init`
  *is* what is being invoked, and it scaffolds the bare core itself before any
  pack transaction.
- **`wf-browser-qa` is disabled:** the seed is reported *not applied*; the pack
...
- **`wf-browser-qa` is already set up and the project is settled:** the canonical
...
- **No test credentials are on disk yet:** not this skill's concern and not a
  lifecycle question. `/wf-browser-qa:qa-engine` obtains them on its own first run,
  when it needs them.
...
- **The project already has other packs set up:** they are all preserved. Entering
  through this command **adds** `wf-browser-qa` to them and deregisters nothing —
...
Engine note:
- browser-qa owns the qa-execution engine surface — do not register it alongside another pack claiming that surface, which would overlap partitioned ownership.
- /wf-browser-qa:qa-engine obtains its test credentials on its own first run, not through setup.
```
...
Seed: wf-browser-qa — not applied (alias could not enter the canonical lifecycle)
Reason: <the resolver diagnostic, or the invocation error, verbatim>

Next: resolve the reason above, then re-run /wf-browser-qa:init.
```

**Both blocks are breaking replacements for the previous
`WF-BROWSER-QA-INIT — <status>` block** (MINOR, pre-1.0): one shared route has one
