...
4. Reach preconditions, don't just observe them: clear/seed **browser storage** to the asserted state, then revert in teardown, per the skill's `references/preconditions.md` (the skill obtains it via `resolve_content`, `class: references-template`, `plugin: wf-browser-qa`, `skill: qa-engine`, `ref: preconditions.md` — never a raw `Read` of the plugin-cache path). Mark BLOCKED only when a precondition genuinely cannot be reached.
