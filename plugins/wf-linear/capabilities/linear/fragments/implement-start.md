...
The `linear` capability's fill for the `implement.start` slot (`replace` policy). `/wf:implement`
...
**Role framing.** This fill opens the **`Impl:` child issue** — the durable record of the
implementation pass — and moves both it and the umbrella to In Progress, so a team watching the
tracker sees work start. It never blocks the phase: implementation proceeds whether or not any of
these operations succeed.

**Why a child issue, continuing the artifact family.** `spec.publish`, `plan.publish` and
...
Invoke `update(<impl-item-id>, labels: ["wf-artifact"])` to mark the child as a published workflow
artifact rather than an independent unit of work. This is **best effort**: if the label does not
exist in the workspace, or the patch is rejected for any reason, state one line and continue — a
missing label never fails the phase. Touch no other field.

## Step 5 — Move both items to In Progress

Two `set_status` calls, in this order:

1. `set_status(<impl-item-id>, "In Progress")` — the implementation record is now active work.
2. `set_status(<umbrella-id>, "In Progress")` — the task as a whole is now being implemented.

The umbrella transition is the one this fill makes to a **shared** item, and it is safe: `In
Progress` is a non-terminal, mid-conveyor state that no other fill and no other skill sets. `tf`
later moves the umbrella to its terminal status; `implement.finish` moves it to In Review in
between. The three transitions are strictly ordered in time and strictly disjoint in value, so the
umbrella lifecycle is never double-driven.
...
summary (e.g. `Opened Impl: <task title> as <impl-item-id> under <umbrella-id>; both In Progress`).
...
| either `set_status` fails | state one line, continue — the record exists either way |
...
fills, and the `tf` reconciliation in full:
