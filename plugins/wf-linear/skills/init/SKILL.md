---
name: init
description: Onboards the wf-linear pack by entering the canonical /wf:init lifecycle with wf-linear seeded into the selection round, so a project gets the same discovery, question, delta, confirmation and apply it would get from /wf:init itself. Preserves every registration the project already has and adds wf-linear to them; it decides nothing about the pack's state, runs no interview of its own, and performs no registry write. The linear capability supplies the tracker provider surface. Use after /wf:init to activate Linear tracker binding; re-run any time — a re-run over a settled project reports no drift and mutates nothing. /wf:init is the canonical command and does the same thing for every pack at once.
allowed-tools: [Skill, Bash]
---

# /wf-linear:init — Onboard the wf-linear pack (a compatibility alias onto the shared lifecycle)

This skill is a **compatibility alias**: it contributes exactly one thing to the
canonical setup lifecycle — "add `wf-linear` to the desired set" — and then gets
...
> **What this skill does not decide.** Whether `wf-linear` is installed, enabled,
...
them makes a question disappear, and neither does the fact that the user typed
this pack's own command. Equally, a question a project has already answered is
**not** asked again — the round asks exactly what is still unresolved, across the
whole desired set rather than just this pack.
...
- Write or edit **any** file, including `_local/config.md` and its `## Linear`
  section. This skill performs no write at all.
...
1. **Route the edge, then enter the lifecycle.** Run `pwd -P` once and hold the
   ...
   `workspaceRoot: "<workspace-root>"`, `role: "init"`, `unitIds: ["linear:init"]`,
   ...
   no selector, and invoke `/wf:init --seed wf-linear` through the **Skill** tool.
...
- **`/wf:init` has not run yet:** not a precondition this skill checks. `/wf:init`
  *is* what is being invoked, and it scaffolds the bare core itself before any
  pack transaction.
- **`wf-linear` is disabled:** the seed is reported *not applied*; the pack stays
...
- **`wf-linear` is already set up and the project is settled:** the canonical
...
- **Linear Team is already answered:** the canonical round does not ask it again.
  A question is re-asked only when no persisted project answer exists at its
  declared destination.
- **Linear Team has never been answered:** the canonical round asks it, even
  though the pack ships a template and a personal profile may suggest a value —
  a suggestion is a pre-fill, never an answer.
...
- **The project already has other packs set up:** they are all preserved. Entering
  through this command **adds** `wf-linear` to them and deregisters nothing —
...
- **`ado` is also in the desired set:** both capabilities claim the `tracker`
...
Tracker note:
- linear owns the tracker surface — do not register it alongside another tracker pack, which would overlap partitioned ownership.
```
...
Seed: wf-linear — not applied (alias could not enter the canonical lifecycle)
Reason: <the resolver diagnostic, or the invocation error, verbatim>

Next: resolve the reason above, then re-run /wf-linear:init.
```

**Both blocks are breaking replacements for the previous `WF-LINEAR-INIT — <status>`
