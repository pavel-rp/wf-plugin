...
| `index` | `plugins/wf/skills/index/SKILL.md` | core | `adaptive` | `inherit` | `inherit` | `2` | WF-379 removed the `index` agent and inlined the single-row write into the caller's context; the `/wf:index` skill routes `inline` with both selectors unset. |
