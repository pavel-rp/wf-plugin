...
| <a id="cal-index"></a>`CAL-index` | `index` | WF-379 | 2026-07-23 | `inherit` | `inherit, haiku` | `missing` | `missing` | `missing` | `resolved` | `none — role inlined` | `retain` | WF-379 removed the `index` agent and inlined the single-row write into the caller's context; no static default is introduced, so the role routes `inline` with selectors unset. |
